...
public class TEInputStream {
  ...
  private void readComponents() {
    int tsSize = ((256 * this.song.getTimeSignature().getNumerator()) / this.song
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (((data[2] & 0xff) & 0x1f) > 0 && ((data[2] & 0xff) & 0x1f) <= 25) {
        ...
        this.song.getComponents().add(
            new TEComponentNote(position, measure, string, fret, duration,
                dynamic, effect));
        ...
        this.song.addTimeSignatureChange(new TETimeSignatureChange(measure,
            new TETimeSignature(numerator, denominator)));
      } else if (((data[2] & 0xff) & 0x1f) == 28 && (data[2] & 0x20) != 0) {
        // SCALE DIAGRAM
      } else if (((data[2] & 0xff) & 0x1f) == 28) {
        // CHORD
        this.song.getComponents().add(
            new TEComponentChord(position, measure, string, data[3]));
      } else if (((data[2] & 0xff) & 0x1f) == 29) {
        // SCALE DIAGRAM | SPECIAL CHAR | SYNCOPATION CHANGE
      } else if (((data[2] & 0xff) & 0x1f) == 30) {
        // TEMPO CHANGE | VOICE CHANGE | DRUM EVENT | CRESCENDO ACCENT |
        // REPEAT/ENDING
      }
      mIndex = measure;
    }
  ...
  private void readNotes(boolean notes) {
    if (notes) {
      ...
      this.song.getInfo().setNotes(
          new String(this.readBytes(length), 1, (length - 1)));
    }
  }
  ...
  private void readTracks() {
    for (int i = 0; i < this.song.getTracks().length; i++) {
      ...
      this.song.setTrack(i, new TETrack((type == 98), instrument, capo,
          clefType, clefNumber, pan, volume, flags, strings, name));
      this.skip(2);
    }
  }
