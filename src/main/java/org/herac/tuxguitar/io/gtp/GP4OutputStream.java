...
public class GP4OutputStream extends GTPOutputStream {
  ...
  private void writeNote(TGNote note) throws IOException {
    ...
    if (note.getEffect().isVibrato() || note.getEffect().isBend()
        || note.getEffect().isGrace() || note.getEffect().isSlide()
        || note.getEffect().isHammer() || note.getEffect().isLetRing()
        || note.getEffect().isPalmMute() || note.getEffect().isStaccato()
        || note.getEffect().isTapping() || note.getEffect().isSlapping()
        || note.getEffect().isPopping() || note.getEffect().isHarmonic()
        || note.getEffect().isTrill() || note.getEffect().isTremoloPicking()) {
      flags |= 0x08;
    }
    writeUnsignedByte(flags);
