...
public class TGMeasureManager {
  ...
  public void changeDuration(TGMeasure measure, TGBeat beat,
    ...
    if (validateDuration(measure, beat, voice, duration, tryMove, false)) {
      ...
      beat.getVoice(voice).setDuration(
  ...
  public List getSortedStringsByValue(TGTrack track, final int direction) {
    ...
    Collections.sort(strings, new Comparator() {
      public int compare(Object o1, Object o2) {
        if (o1 != null && o2 != null && o1 instanceof TGString
            && o2 instanceof TGString) {
    ...
    if (newMeasure != null) {
      ...
      for (int v = 0; v < beat.countVoices(); v++) {
        ...
        if (!voice.isEmpty() && (bStart + vDuration) > (mStart + mLength)) {
          ...
          if (vDuration > 0) {
            TGDuration duration = TGDuration.fromTime(getSongManager()
                .getFactory(), vDuration);
            if (duration != null) {
              duration.copy(voice.getDuration());
            }
          }
          if (vTiedDuration > 0) {
            TGDuration newVoiceDuration = TGDuration.fromTime(getSongManager()
                .getFactory(), vTiedDuration);
  ...
  public void moveVoice(TGVoice voice, TGBeat beat) {
    ...
    if (!currentBeat.equals(beat)) {
      if (currentBeat.getVoice(voice.getIndex()).equals(voice)) {
        ...
        TGVoice newVoice = getSongManager().getFactory().newVoice(
            voice.getIndex());
  ...
  public boolean moveVoices(TGMeasure measure, long start, long theMove,
    ...
    List voicesToMove = getVoicesBeforeEnd(measure.getBeats(), start,
        voiceIndex);
  ...
  private boolean transposeNote(TGNote note, List notes, List strings,
  ...
  public void tryChangeSilenceAfter(TGMeasure measure, TGVoice voice) {
    ...
    if (nextVoice != null && !nextVoice.isEmpty() && nextVoice.isRestVoice()
      ...
      if ((nextVoice.getBeat().getStart() + theMove) < measureEnd
        ...
        changeDuration(measure, nextVoice.getBeat(), voice.getDuration().clone(
            getSongManager().getFactory()), nextVoice.getIndex(), false);
      }
    }
  }
    ...
    if (moveNextBeats) {
      ...
      if (nextVoice != null) {
        ...
        if (requiredLength <= (nextSilenceLength + errorMargin)) {
          List voices = getVoicesBeforeEnd(measure.getBeats(), nextVoice
              .getBeat().getStart(), voice);
