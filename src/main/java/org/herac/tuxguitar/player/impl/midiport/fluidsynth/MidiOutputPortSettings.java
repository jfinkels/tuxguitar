...
public class MidiOutputPortSettings extends MidiSettings {
  ...
  protected void addMidiPorts(Table table) {
    for (final String path : getSoundfonts()) {
  ...
  public void configure(Shell parent) {
    ...
    tSoundfonts
        .setText(TuxGuitar.getProperty("fluidsynth.settings.soundfonts"));
    ...
    final Table soundfontsTable = new Table(cSoundfontsTable, SWT.BORDER
    ...
    TableColumn soundfontsTableColumn = new TableColumn(soundfontsTable,
        SWT.NONE);
    soundfontsTableColumn.setWidth(TABLE_WIDTH);
    soundfontsTableColumn.setText(TuxGuitar
        .getProperty("fluidsynth.settings.soundfonts.list"));

    Composite cSoundfontsButtons = new Composite(cSoundfonts, SWT.NONE);
    cSoundfontsButtons.setLayout(new GridLayout());
    cSoundfontsButtons.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    ...
    lAudioDriver.setText(TuxGuitar
        .getProperty("fluidsynth.settings.audio.driver"));
    ...
    // Audio Sample Format -----------------------------------------------------
    final List<String> audioSampleFormatOptions = this
        .getAudioSampleFormatOptions();

    Label lAudioSampleFormat = new Label(cAudio, SWT.NONE);
    lAudioSampleFormat.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
        false));
    lAudioSampleFormat.setText(TuxGuitar
        .getProperty("fluidsynth.settings.audio.sample-format"));

    final Combo cAudioSampleFormat = new Combo(cAudio, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    cAudioSampleFormat.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        false));

    fillAudioSampleFormatCombo(audioSampleFormatOptions, cAudioSampleFormat);

    // Audio Period Size -------------------------------------------------------
    final List<Integer> audioPeriodSizeOptions = this
        .getAudioPeriodSizeOptions();

    Label lAudioPeriodSize = new Label(cAudio, SWT.NONE);
    lAudioPeriodSize.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
        false));
    lAudioPeriodSize.setText(TuxGuitar
        .getProperty("fluidsynth.settings.audio.period-size"));

    final Combo cAudioPeriodSize = new Combo(cAudio, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    cAudioPeriodSize
    ...
    // Audio Period Count
    // -------------------------------------------------------
    final List<Integer> audioPeriodCountOptions = this
        .getAudioPeriodCountOptions();

    Label lAudioPeriodCount = new Label(cAudio, SWT.NONE);
    lAudioPeriodCount.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
        false));
    lAudioPeriodCount.setText(TuxGuitar
        .getProperty("fluidsynth.settings.audio.periods"));

    final Combo cAudioPeriodCount = new Combo(cAudio, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    cAudioPeriodCount.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        false));
    ...
    // Synth Sample Rate -------------------------------------------------------
    final List<Double> synthSampleRateOptions = this
        .getSynthSampleRateOptions();

    Label lSynthSampleRate = new Label(cSynth, SWT.NONE);
    lSynthSampleRate.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
        false));
    lSynthSampleRate.setText(TuxGuitar
        .getProperty("fluidsynth.settings.synth.sample-rate"));

    final Combo cSynthSampleRate = new Combo(cSynth, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    cSynthSampleRate
        .setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
    ...
    lSynthPolyphony.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
        false));
    lSynthPolyphony.setText(TuxGuitar
        .getProperty("fluidsynth.settings.synth.polyphony"));
    ...
    bSynthReverbActive.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        false, 2, 1));
    bSynthReverbActive.setText(TuxGuitar
        .getProperty("fluidsynth.settings.synth.reverb.active"));
    fillSynthReverbActiveCheckbox(bSynthReverbActive);

    // Synth Chorus -------------------------------------------------------
    final Button bSynthChorusActive = new Button(cSynth, SWT.CHECK);
    bSynthChorusActive.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        false, 2, 1));
    bSynthChorusActive.setText(TuxGuitar
        .getProperty("fluidsynth.settings.synth.chorus.active"));
    ...
    buttonOK.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent arg0) {
        ...
        updateAudioDriverSelection(audioDriverOptions, cAudioDriver
            .getSelectionIndex());
        updateAudioSampleFormatSelection(audioSampleFormatOptions,
            cAudioSampleFormat.getSelectionIndex());
        updateAudioPeriodSizeSelection(audioPeriodSizeOptions, cAudioPeriodSize
            .getSelectionIndex());
        updateAudioPeriodCountSelection(audioPeriodCountOptions,
            cAudioPeriodCount.getSelectionIndex());
        updateSynthSampleRateSelection(synthSampleRateOptions, cSynthSampleRate
            .getSelectionIndex());
  ...
  protected void fillSynthGainScale(Scale scale) {
    ...
    if (range.length == 2) {
      int value = (int) Math
          .round(getDoubleValue(MidiSettings.SYNTH_GAIN) * 10f);
  ...
  protected void fillSynthPolyphonySpinner(Spinner spinner) {
    ...
    int[] range = getSynth().getIntegerPropertyRange(
        MidiSettings.SYNTH_POLYPHONY);
  ...
  protected List<Integer> getAudioPeriodCountOptions() {
    ...
    int[] range = getSynth().getIntegerPropertyRange(
        MidiSettings.AUDIO_PERIOD_COUNT);
    if (range.length == 2 && range[0] < range[1]) {
      int value = range[0];
      while (value <= range[1]) {
  ...
  protected List<Integer> getAudioPeriodSizeOptions() {
    List<Integer> options = new ArrayList<Integer>();
    int[] range = getSynth().getIntegerPropertyRange(
        MidiSettings.AUDIO_PERIOD_SIZE);
    if (range.length == 2 && range[0] < range[1]) {
      int value = range[0];
      while (value <= range[1]) {
        options.add(new Integer(value));
  ...
  protected GridData getButtonData(int hAlignment, int vAlignment,
      boolean grabExcessHSpace, boolean grabExcessVSpace) {
    GridData data = new GridData(hAlignment, vAlignment, grabExcessHSpace,
        grabExcessVSpace);
  ...
  protected List<Double> getSynthSampleRateOptions() {
    ...
    double[] range = getSynth().getDoublePropertyRange(
        MidiSettings.SYNTH_SAMPLE_RATE);
  ...
  protected void updateAudioPeriodCountSelection(List<Integer> options,
      int index) {
    if (index >= 0 && index < options.size()) {
  ...
  protected void updateAudioSampleFormatSelection(List<String> options,
      int index) {
    if (index >= 0 && index < options.size()) {
  ...
  protected void updateSynthPolyphonySelection(int value) {
    int[] range = getSynth().getIntegerPropertyRange(
        MidiSettings.SYNTH_POLYPHONY);
    if (range.length == 2 && range[0] < range[1] && value >= range[0]
        && value <= range[1]) {
      if (value >= range[0] && value <= range[1]) {
