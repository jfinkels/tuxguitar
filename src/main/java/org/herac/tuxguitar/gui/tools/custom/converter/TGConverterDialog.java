...
public class TGConverterDialog implements LanguageLoader, IconLoader {
      ...
      if (extensions != null && extensions.length > 0) {
        for (int i = 0; i < extensions.length; i++) {
          ...
          if (dotIndex >= 0) {
            ...
            this.outputFormats.add(new TGConverterFormat((extensions[i]
                .substring(dotIndex)).trim(), exporter));
          }
        }
      }
    }
  ...
  public void loadIcons(boolean layout) {
    if (!isDisposed()) {
      ...
      this.inputFolderChooser.setImage(TuxGuitar.instance().getIconManager()
          .getFileOpen());
      this.outputFolderChooser.setImage(TuxGuitar.instance().getIconManager()
          .getFileOpen());
      if (layout) {
  ...
  public void loadProperties(boolean layout) {
    if (!isDisposed()) {
      ...
      this.inputFolderLabel.setText(TuxGuitar
          .getProperty("batch.converter.input.folder"));
      this.outputFolderLabel.setText(TuxGuitar
          .getProperty("batch.converter.output.folder"));
      this.outputFormatLabel.setText(TuxGuitar
          .getProperty("batch.converter.output.format"));
  ...
  public void show() {
    ...
    this.outputFormatLabel.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER,
        false, false));

    final Combo outputFormat = new Combo(composite, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    outputFormat.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false, 2,
        1));
    addFileFormats(outputFormat);

    this.inputFolderLabel = new Label(composite, SWT.LEFT);
    this.inputFolderLabel.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER,
        false, false));
    ...
    this.outputFolderLabel.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER,
        false, false));
    ...
    this.buttonOK.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent arg0) {
        ...
        TGConverterFormat outputFormatValue = getFileFormat(outputFormat
            .getSelectionIndex());

        if (inputFolderValue == null || inputFolderValue.trim().length() == 0) {
          MessageDialog.errorMessage(TGConverterDialog.this.dialog, TuxGuitar
              .getProperty("batch.converter.input.folder.invalid"));
        } else if (outputFolderValue == null
            || outputFolderValue.trim().length() == 0) {
          MessageDialog.errorMessage(TGConverterDialog.this.dialog, TuxGuitar
              .getProperty("batch.converter.output.folder.invalid"));
        } else if (outputFormat == null) {
          MessageDialog.errorMessage(TGConverterDialog.this.dialog, TuxGuitar
              .getProperty("batch.converter.output.format.invalid"));
        } else {
          TGConverterProcess process = new TGConverterProcess();
          process.start(inputFolderValue.trim(), outputFolderValue.trim(),
              outputFormatValue);
