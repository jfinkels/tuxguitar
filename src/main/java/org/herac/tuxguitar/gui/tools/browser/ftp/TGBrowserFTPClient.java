...
public class TGBrowserFTPClient {
  ...
  public synchronized void open(String host, int port) throws IOException {
    ...
    this.response = new BufferedReader(new InputStreamReader(this.socket
        .getInputStream()));
    this.request = new BufferedWriter(new OutputStreamWriter(this.socket
        .getOutputStream()));
  ...
  private synchronized byte[] sendPasvCMD(String cmd) throws IOException {
    ...
    if (isExpectedResponse(response, RESPONSE_CODE_PASV)) {
      ...
      if (addressIndex1 > 0 && addressIndex2 > addressIndex1) {
        String[] address = response.substring(addressIndex1, addressIndex2)
            .split(",");
        if (address.length == 6) {
          ...
          Socket socket = new Socket((a1 + "." + a2 + "." + a3 + "." + a4),
              ((p1 * 256) + p2));
