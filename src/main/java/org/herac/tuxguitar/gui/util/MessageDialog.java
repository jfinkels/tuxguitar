...
public class MessageDialog {
      ...
      new SyncThread(new Runnable() {
        public void run() {
          if (!shell.isDisposed()) {
            ...
            new MessageDialog(TuxGuitar.getProperty("error"), message,
                SWT.ICON_ERROR).show(shell);
          }
        }
      }).start();
    }
  ...
  public static void errorMessage(final Shell shell, final Throwable throwable) {
    MessageDialog.errorMessage(shell,
  ...
  public static void infoMessage(final Shell shell, final String title,
