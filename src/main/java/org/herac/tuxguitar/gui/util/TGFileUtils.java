...
public class TGFileUtils {
    ...
    try {
      if (TG_STATIC_SHARED_PATHS != null) {
        for (int i = 0; i < TG_STATIC_SHARED_PATHS.length; i++) {
          File file = new File(TG_STATIC_SHARED_PATHS[i] + File.separator
              + resource);
          if (file.exists()) {
  ...
  public static List<URL> getResourceUrls(String resource) {
    try {
      ...
      Enumeration<URL> resources = TGClassLoader.instance().getClassLoader()
  ...
  public static Image loadImage(String name) {
    return loadImage(TuxGuitar.instance().getConfig().getStringConfigValue(
        TGConfigKeys.SKIN), name);
  }
  ...
  public static Image loadImage(String skin, String name) {
    try {
      ...
      if (stream != null) {
        return new Image(TuxGuitar.instance().getDisplay(), new ImageData(
            stream));
      }
  ...
  public static void loadLibraries() {
    ...
    if (libraryPath != null) {
      ...
      for (int i = 0; i < libraryPaths.length; i++) {
        TGLibraryLoader.instance().loadLibraries(new File(libraryPaths[i]),
            libraryPrefix, libraryExtension);
      }
    }
  }
}
