...
public class ConfirmDialog {
  ...
  private void addCloseButton(String text, final int value, Composite parent,
  ...
  public int confirm(int style, int defaultButton) {
    ...
    if ((style & BUTTON_YES) != 0) {
      addCloseButton(TuxGuitar.getProperty("yes"), STATUS_YES,
          buttonsComposite, (defaultButton == BUTTON_YES));
      buttonsLayout.numColumns++;
    }
    if ((style & BUTTON_NO) != 0) {
      addCloseButton(TuxGuitar.getProperty("no"), STATUS_NO, buttonsComposite,
          (defaultButton == BUTTON_NO));
      buttonsLayout.numColumns++;
    }
    if ((style & BUTTON_CANCEL) != 0) {
      addCloseButton(TuxGuitar.getProperty("cancel"), STATUS_CANCEL,
          buttonsComposite, (defaultButton == BUTTON_CANCEL));
      buttonsLayout.numColumns++;
    }
