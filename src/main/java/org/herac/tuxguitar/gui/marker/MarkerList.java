...
public class MarkerList implements TGUpdateListener, IconLoader, LanguageLoader {
  ...
  protected void loadTableItems(boolean keepSelection) {
    ...
    for (final TGMarker marker : this.markers){
      ...
      item.setText(new String[] { Integer.toString(marker.getMeasure()),
          marker.getTitle() });
    }
  ...
  private GridData makeGridData(int horizontalAlignment, int verticalAlignment,
  ...
  public void show() {
    ...
    this.compositeTable.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    ...
    this.compositeButtons.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    ...
    this.buttonEdit.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent arg0) {
        if (!ActionLock.isLocked() && !TuxGuitar.instance().isLocked()) {
          ...
          if (marker != null) {
            if (new MarkerEditor(marker, MarkerEditor.STATUS_EDIT)
                .open(MarkerList.this.dialog)) {
              TuxGuitar.instance().updateCache(true);
              loadTableItems(true);
            }
    ...
    this.buttonDelete.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent arg0) {
        if (!ActionLock.isLocked() && !TuxGuitar.instance().isLocked()) {
          ...
          UndoableChangeMarker undoable = UndoableChangeMarker
              .startUndo(marker);
