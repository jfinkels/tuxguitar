...
public class MarkerEditor {
  ...
  public boolean open(Shell shell) {
    ...
    // Measure Number
    final int measureCount = TuxGuitar.instance().getSongManager().getSong()
        .countMeasureHeaders();
    ...
    this.measureSpinner.setLayoutData(getAlignmentData(MINIMUM_CONTROL_WIDTH,
        SWT.FILL));
    ...
    this.titleText.setLayoutData(getAlignmentData(MINIMUM_CONTROL_WIDTH,
        SWT.FILL));
    ...
    this.colorButton.setLayoutData(getAlignmentData(MINIMUM_CONTROL_WIDTH,
        SWT.FILL));
  ...
  protected void updateMarker() {
    ...
    if (this.status == STATUS_EDIT && oldMeasure != this.marker.getMeasure()) {
      UndoableChangeMarker undoable = UndoableChangeMarker.startUndo(manager
          .getMarker(oldMeasure));
      TuxGuitar.instance().getSongManager().removeMarker(oldMeasure);
      joinedUndoable.addUndoableEdit(undoable.endUndo(null));
    }
    UndoableChangeMarker undoable = UndoableChangeMarker.startUndo(manager
        .getMarker(this.marker.getMeasure()));
