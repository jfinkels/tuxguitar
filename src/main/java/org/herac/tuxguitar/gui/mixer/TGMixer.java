...
public class TGMixer implements TGUpdateListener, IconLoader, LanguageLoader {
  ...
  protected void changeVolume() {
    int volume = (short) (TGMixer.this.volumeScale.getMaximum() - TGMixer.this.volumeScale
        .getSelection());
    if (volume != TuxGuitar.instance().getPlayer().getVolume()) {
      TuxGuitar.instance().getPlayer().setVolume(volume);
      this.volumeScale.setToolTipText(TGMixer.this.volumeTip + ": "
          + TuxGuitar.instance().getPlayer().getVolume());
      this.volumeValueLabel.setText(Integer.toString(TGMixer.this.volumeScale
          .getMaximum()
          - TGMixer.this.volumeScale.getSelection()));
  ...
  protected void loadData() {
    ...
    this.volumeScale.setLayoutData(new GridData(SWT.CENTER, SWT.FILL, true,
        true));
  ...
  public synchronized void loadProperties(boolean pack) {
    if (!isDisposed()) {
      ...
      this.volumeScale.setToolTipText(this.volumeTip + ": "
          + TuxGuitar.instance().getPlayer().getVolume());
  ...
  protected void loadVolume() {
    ...
    if (this.volumeValue != volume) {
      this.volumeScale.setSelection(this.volumeScale.getMaximum()
          - TuxGuitar.instance().getPlayer().getVolume());
      this.volumeValueLabel.setText(Integer.toString(this.volumeScale
          .getMaximum()
