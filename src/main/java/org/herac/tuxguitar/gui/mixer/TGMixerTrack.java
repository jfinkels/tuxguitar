...
public class TGMixerTrack {
  ...
  private class TGMixerScale {
    ...
    public TGMixerScale(Composite parent, int style, int pageIncrement,
        int type, Object layoutData) {
        ...
        public void mouseUp(MouseEvent arg0) {
          if (TGMixerScale.this.undoable != null) {
            TuxGuitar.instance().getUndoableManager().addEdit(
                TGMixerScale.this.undoable.endUndo());
    ...
    public void init(Composite parent, int style, int pageIncrement, int type,
        Object layoutData) {
    ...
    public void setSelection(int value) {
      if (value != this.value) {
        this.value = value;
        this.scale
            .setSelection((this.inverted ? 127 - this.value : this.value));
    ...
    public void updateValue() {
      this.setSelection((this.inverted ? 127 - this.scale.getSelection()
          : this.scale.getSelection()));
    }
  }
  ...
  private class TGMixerScalePopup extends TGMixerScale {
    ...
    public TGMixerScalePopup(Composite parent, int style, int pageIncrement,
        int type, Object layoutData) {
    ...
    public void showSelect() {
      if (!this.shell.isVisible()) {
        ...
        Point location = this.item.getParent().toDisplay(
            new Point(bounds.x, bounds.y));

        this.shell.pack();
        this.shell.setLocation((location.x + (bounds.width / 2))
            - (this.shell.getSize().x / 2), location.y + bounds.height);
  ...
  public void init(final Composite parent) {
    ...
    this.checkSolo.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent e) {
        ...
        TuxGuitar.instance().getSongManager().getTrackManager().changeSolo(
            track, TGMixerTrack.this.checkSolo.getSelection());
        TGMixerTrack.this.mixer.fireChanges(track.getChannel(), TGMixer.SOLO);
        TuxGuitar.instance().getUndoableManager().addEdit(
            undoable.endUndo(track));
        TuxGuitar.instance().updateCache(true);
      }
    });
    ...
    this.checkMute.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent e) {
        ...
        TuxGuitar.instance().getSongManager().getTrackManager().changeMute(
            track, TGMixerTrack.this.checkMute.getSelection());
    ...
    this.scaleChorus = new TGMixerScalePopup(effects, SWT.VERTICAL, 64,
        TGMixer.CHORUS, getVerticalScaleData());
    this.scaleChorus.setSelection(this.track.getChannel().getChorus());

    this.scaleReverb = new TGMixerScalePopup(effects, SWT.VERTICAL, 64,
        TGMixer.REVERB, getVerticalScaleData());
    this.scaleReverb.setSelection(this.track.getChannel().getReverb());

    this.scalePhaser = new TGMixerScalePopup(effects, SWT.VERTICAL, 64,
        TGMixer.PHASER, getVerticalScaleData());
    this.scalePhaser.setSelection(this.track.getChannel().getPhaser());

    this.scaleTremolo = new TGMixerScalePopup(effects, SWT.VERTICAL, 64,
        TGMixer.TREMOLO, getVerticalScaleData());
    this.scaleTremolo.setSelection(this.track.getChannel().getTremolo());

    new Label(composite, SWT.HORIZONTAL | SWT.SEPARATOR)
        .setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));

    this.scaleBalance = new TGMixerScale(composite, SWT.HORIZONTAL, 64,
        TGMixer.BALANCE, getHorizontalScaleData());
    this.scaleBalance.setSelection(this.track.getChannel().getBalance());

    this.scaleVolume = new TGMixerScale(composite, SWT.VERTICAL, 16,
        TGMixer.VOLUME, new GridData(SWT.CENTER, SWT.FILL, true, true));
    ...
    this.volumeValueLabel.setText(Integer.toString(this.track.getChannel()
        .getVolume()));
  }
  ...
  public void loadProperties() {
    ...
    this.volumeValueTitleLabel.setText(TuxGuitar
        .getProperty("mixer.channel.volume")
        + ":");
