...
public class KeyBindingEditor {
  ...
  public boolean exists(KeyBinding kb) {
    for (final TableItem item : this.items) {
      if (item.getData() instanceof KeyBindingAction) {
        ...
        if (itemData.getKeyBinding() != null
            && kb.isSameAs(itemData.getKeyBinding())) {
          return true;
        }
      }
    }
  ...
  protected void loadAvailableActionKeyBindings() {
    List<String> list = TuxGuitar.instance().getActionManager()
  ...
  protected void loadTableItemLabel(TableItem item) {
    if (item.getData() instanceof KeyBindingAction) {
      ...
      String shortcut = (actionkeyBinding.getKeyBinding() != null) ? actionkeyBinding
          .getKeyBinding().toString()
          : "";
  ...
  protected void save() {
    ...
    for (final TableItem item : this.items) {
      if (item.getData() instanceof KeyBindingAction) {
        ...
        if (keyBindingAction.getAction() != null
            && keyBindingAction.getKeyBinding() != null) {
  ...
  public void show(Shell parent) {
    ...
    this.table.addMouseListener(new MouseAdapter() {
      public void mouseDoubleClick(MouseEvent e) {
        ...
        if (item != null) {
          KeyBindingAction itemData = (KeyBindingAction) item.getData();
          KeyBindingSelector selector = new KeyBindingSelector(
              KeyBindingEditor.this, itemData);
          KeyBinding kb = selector.select(KeyBindingEditor.this.dialog
              .getShell());
    ...
    actionColumn.setText(TuxGuitar
        .getProperty("key-bindings-editor-action-column"));

    TableColumn shortcutColumn = new TableColumn(this.table, SWT.LEFT);
    shortcutColumn.setText(TuxGuitar
        .getProperty("key-bindings-editor-shortcut-column"));

    loadAvailableActionKeyBindings();
    loadEnableActionKeyBindings(TuxGuitar.instance().getkeyBindingManager()
    ...
    defaults.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent e) {
        loadEnableActionKeyBindings(KeyBindingActionDefaults
    ...
    this.table.setLayoutData(new GridData((adjustWidth(actionColumn,
        ACTION_WIDTH) + adjustWidth(shortcutColumn, SHORTCUT_WIDTH)), 250));
