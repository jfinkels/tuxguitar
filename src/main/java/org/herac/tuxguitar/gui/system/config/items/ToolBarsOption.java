...
public class ToolBarsOption extends Option {
  ...
  public ToolBarsOption(TGConfigEditor configEditor, ToolBar toolBar,
      final Composite parent) {
    super(configEditor, toolBar, parent, TuxGuitar
        .getProperty("settings.config.toolbars"), SWT.FILL, SWT.FILL);
    this.initialized = false;
  }

  public void applyConfig(boolean force) {
    if (force
        || (this.initialized && TuxGuitar.instance().getItemManager()
  ...
  public void createOption() {
    ...
    this.moveUp.setText(TuxGuitar
        .getProperty("settings.config.toolbars.move-up"));
    ...
    this.moveDown.setText(TuxGuitar
        .getProperty("settings.config.toolbars.move-down"));
  ...
  protected void loadConfig() {
    new Thread(new Runnable() {
      public void run() {
        final ToolItems[] items = TuxGuitar.instance().getItemManager()
            .getToolBars();
        new SyncThread(new Runnable() {
          public void run() {
            if (!isDisposed()) {
              for (int i = 0; i < items.length; i++) {
                loadItem(new TableItem(ToolBarsOption.this.table, SWT.NONE),
                    items[i]);
              }
  ...
  public void updateConfig() {
    if (this.initialized) {
      for (int i = 0; i < this.table.getItemCount(); i++) {
        ...
        TuxGuitar.instance().getItemManager().setToolBarStatus(data.getName(),
            item.getChecked(), i);
      }
