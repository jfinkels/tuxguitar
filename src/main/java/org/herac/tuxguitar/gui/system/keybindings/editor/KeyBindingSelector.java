...
public class KeyBindingSelector {
  ...
  protected boolean isValid(KeyBinding kb) {
    if (KeyBindingReserveds.isReserved(kb)) {
      if (!this.editor.isDisposed()) {
        String title = TuxGuitar
            .getProperty("key-bindings-editor-reserved-title");
        String message = TuxGuitar
            .getProperty("key-bindings-editor-reserved-message");
    ...
    if (this.editor.exists(kb)) {
      ConfirmDialog confirm = new ConfirmDialog(TuxGuitar
          .getProperty("key-bindings-editor-override"));
  ...
  public KeyBinding select(Shell parent) {
    this.dialog = DialogUtils.newDialog(parent, SWT.APPLICATION_MODAL
        | SWT.DIALOG_TRIM);
    ...
    iconLabel.setImage(iconLabel.getDisplay().getSystemImage(
    ...
    if (fd != null && fd.length > 0) {
      final Font font = new Font(textLabel.getDisplay(), new FontData(fd[0]
          .getName(), 14, SWT.BOLD));
