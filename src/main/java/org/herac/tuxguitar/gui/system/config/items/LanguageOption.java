...
public class LanguageOption extends Option {
  ...
  public LanguageOption(TGConfigEditor configEditor, ToolBar toolBar,
      final Composite parent) {
    super(configEditor, toolBar, parent, TuxGuitar
        .getProperty("settings.config.language"), SWT.FILL, SWT.FILL);
  ...
  public void applyConfig(boolean force) {
    if (force || this.initialized) {
      ...
      if (!changed) {
        String languageLoaded = TuxGuitar.instance().getLanguageManager()
            .getLanguage();
        String languageConfigured = getConfig().getStringConfigValue(
      ...
      public void run() {
        ...
        new SyncThread(new Runnable() {
          public void run() {
            if (!isDisposed()) {
              // Load default item
              loadTableItem(TuxGuitar.getProperty("locale.default"),
                  new String(), true);

              for (int i = 0; i < languages.size(); i++) {
                LanguageItem item = (LanguageItem) languages.get(i);
                loadTableItem(item.getValue(), item.getKey(),
                    (language != null && item.getKey().equals(language)));
              }
  ...
  public void updateDefaults() {
    if (this.initialized) {
      getConfig().setProperty(TGConfigKeys.LANGUAGE,
          getDefaults().getProperty(TGConfigKeys.LANGUAGE));
    }
  }
}