...
public class LinearViewLayout extends ViewLayout {
  ...
  public void paintMeasures(TGTrackImpl track, TGPainter painter, int fromX,
      int fromY, TGTrackSpacing ts, Rectangle clientArea) {
    ...
    for (final TGMeasure meas : track.getMeasures()) {
      ...
      boolean isAtX = ((posX + measure.getWidth(this)) > clientArea.x - 100 && posX < clientArea.x
          + clientArea.width + measure.getWidth(this) + 100);
  ...
  public void paintSong(TGPainter painter, Rectangle clientArea, int fromX,
    ...
    for (final TGTrack trk : getSongManager().getSong().getTracks()) {
      ...
      if (number < 0 || track.getNumber() == number) {
        ...
        ts.setSize(TGTrackSpacing.POSITION_SCORE_UP_LINES,
            ((style & DISPLAY_SCORE) != 0 ? Math.abs(minY) : 0));
        if ((style & DISPLAY_SCORE) != 0
            && maxY + getMinScoreTabSpacing() > getScoreSpacing()) {
          ts.setSize(TGTrackSpacing.POSITION_SCORE_DOWN_LINES,
              (maxY - (getScoreLineSpacing() * 4)));
        }
        ...
        paintLines(track, ts, painter, fromX + (getWidth() + 2), posY,
            (clientArea.width - (fromX + getWidth())));
