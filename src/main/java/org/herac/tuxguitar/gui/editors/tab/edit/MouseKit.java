...
public class MouseKit {
  ...
  public TGVoiceImpl findBestVoice(TGMeasureImpl measure, int x) {
    ...
    int posX = measure.getHeaderImpl().getLeftSpacing(
        this.kit.getTablature().getViewLayout())
        + measure.getPosX();
    int bestDiff = -1;
    ...
    for (final TGBeat bt : measure.getBeats()){
      ...
      if (!voice.isEmpty()) {
        ...
        if (voice.isRestVoice()) {
          increment = (duration.getTime() * voice.getWidth() / voice
              .getDuration().getTime());
        }
    ...
    if (bestVoice == null) {
      TGBeat beat = this.kit.getTablature().getViewLayout().getSongManager()
  ...
  private long getRealStart(TGVoiceImpl voice, int x) {
    ...
    long beatX = (measure.getHeaderImpl().getLeftSpacing(
        this.kit.getTablature().getViewLayout())
        + measure.getPosX() + voice.getBeatImpl().getPosX() + voice
    ...
    if (x > beatX) {
      return Math.min(
          (beatStart + ((x - beatX) * beatLength / voice.getWidth())),
          (beatEnd - 1));
    }
    return beatStart;
  }
  ...
  public void mouseMove(MouseEvent e) {
    ...
    if (!TuxGuitar.instance().isLocked() && !ActionLock.isLocked()
        && !this.kit.getTablature().isPainting()) {
      ...
      if (track != null) {
        ...
        if (measure != null && measure.getTs() != null) {
          ...
          int topHeight = measure.getTs().getPosition(
              TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES);
          int bottomHeight = (measure.getTs().getPosition(
              TGTrackSpacing.POSITION_TABLATURE) - measure.getTs().getPosition(
              TGTrackSpacing.POSITION_SCORE_DOWN_LINES));

          int x1 = (int) (e.x - (width / 2.0f));
          int x2 = (int) (e.x + (width / 2.0f));
          int y1 = (measure.getPosY() + measure.getTs().getPosition(
              TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES));
          int y2 = (y1 + (lineSpacing * 5));

          if (e.y < (y1 + 3) && e.y >= (y1 - topHeight)) {
            this.back = new Image(TuxGuitar.instance().getDisplay(), width + 1,
                topHeight + 1);
            TGPainter painter = new TGPainter(new GC(this.kit.getTablature()));
            painter.copyArea(this.back, x1, (y1 - topHeight));
            painter.setForeground(this.kit.getTablature().getViewLayout()
                .getResources().getLineColor());
    ...
    if (!TuxGuitar.instance().isLocked() && !ActionLock.isLocked()
      ...
      if (pos != null) {
        ...
        if (measure.getTs() != null) {
          ...
          int y1 = (pos.getPosY() + measure.getTs().getPosition(
              TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES));
          int y2 = (y1 + (lineSpacing * 5));
  ...
  private void redrawTablature() {
    Caret caret = this.kit.getTablature().getCaret();
    this.kit.getTablature().getViewLayout().fireUpdate(
        caret.getMeasure().getNumber());
