...
public class ChordCreatorUtil {
  ...
  public static final transient Logger LOG = Logger
      .getLogger(ChordCreatorUtil.class);
  ...
  public static void getChords(final ChordCreatorListener listener,
      final int[] tuning, final int chordIndex, final int alteration,
      final int plusMinus, final boolean add, final int add5, final int add9,
      final int add11, final int bassTonic, final int chordTonic,
      final boolean sharp) {

    final ChordCreatorUtil chordCreator = new ChordCreatorUtil(getNewProcess(),
        listener);
    new Thread(new Runnable() {
      public void run() {
        chordCreator.getChords(tuning, chordIndex, alteration, plusMinus, add,
            add5, add9, add11, bassTonic, chordTonic, sharp);
      }
    }).start();
  }
   ...
   * strings,
   * 
   * which is controlled with <i>boolean
   * ChordCreatorUtil.EMPTY_STRING_CHORDS</i>
   * 
   * @param combination
  ...
  private boolean checkCombination(List<StringValue> combination) {
    ...
    for (final StringValue stringValue : combination) {
  ...
  /**
   * We have to make sure that if required note is already inside expanding
   * notes array so we don't put it twice...
   */
   ...
   * 
   * @param stringValues
   *          current StringValue to be examined
   * @param betterOnes
   *          ArrayList of already stored StringList chords
   * @return true if it is duplicate, false if it is unique
   */
  private boolean checkIfSubset(List<StringValue> stringValues,
      List<List<StringValue>> betterOnes) {
    if (!isValidProcess()) {
      return false;
    }

    for (final List<StringValue> currentStringValue : betterOnes) {
      ...
      for (int i = 0; i < currentStringValue.size(); i++) {
        ...
        for (int j = 0; j < stringValues.size(); j++)
          if (stringValues.get(j).getString() == currentString) {
            // if the frets on the same string differ, then chords are not
            // subset/superset of each other
            if (stringValues.get(j).getFret() != currentStringValue.get(i)
                .getFret())
   ...
   * - there appear alteration tones on their specific places<br>
   * <br>
   * 
   * Algorithm:<br>
   * - search for chord tonic. If some note is found before (and it's not bass)
   * do penalty<br>
   * - make penalty if the bass tone is not in bass<br>
   * - check if all the expanding notes are here. If some are not, do penalty<br>
   * - if expanding note isn't higher than tonic octave, then priority should be
   * less<br>
   * - If there are not some with NON_ESSENTIAL_INDEX are not here, penalty
   * should be less<br>
   */
  private float combinationChordSemantics(
      List<StringValue> stringValueCombination) {
    if (!isValidProcess()) {
      return 0;
    }
    float finalGrade = 0;
      ...
      if (current != null) {
        ...
        if (stringDepth > 1) {
          ...
          if (current.getRequiredNoteIndex() < 0) { // expanding tones
            ...
            if (foundTonic == -1)
              finalGrade -= ChordSettings.instance()
                  .getGoodChordSemanticsGrade() / 2;
            else {
              // if expanding note isn't higher than tonic's octave
              if (foundTonic + 11 > this.tuning[current.getString()]
                  + current.getFret())
                finalGrade -= ChordSettings.instance()
                    .getGoodChordSemanticsGrade() / 3;
            }

            // search for distinct expanding notes
            for (int i = 0; i < this.expandingNotes.length; i++)
              if ((this.tuning[string] + current.getFret()) % 12 == (this.chordTonic
                  + this.expandingNotes[i] - 1) % 12)
    ...
    if (this.alteration != 0) {
      ...
      if (essentials + nonEssentials == this.expandingNotes.length)
        ...
        // but if some are missing, that's BAD:
          ...
          finalGrade += (essentials + nonEssentials - this.expandingNotes.length)
              * ChordSettings.instance().getGoodChordSemanticsGrade();
          // half of the penalty for non-essential notes
          finalGrade += nonEssentials
              * ChordSettings.instance().getGoodChordSemanticsGrade() / 2;
        }
      }
    }
  ...
  private float combinationHasAllRequiredNotes(
      List<StringValue> stringValueCombination) {
    if (!isValidProcess()) {
      return 0;
    }
    ...
    if (currentIndex == this.requiredNotes.length - 1) {
      ...
      for (final StringValue current : stringValueCombination) {
      ...
      if (!existsSubdominant && currentIndex == this.requiredNotes.length - 1) {
        // if not riff. "sus" chord, or chord with altered fifth allow chord
        // without JUST subdominant (fifth) with small penalty
    ...
    int noteCount = (this.alteration == 0 ? 0 : 1 + this.alteration)
        + currentIndex + (this.bassTonic == this.chordTonic ? 0 : 1);

    // sometimes, when noteCount is bigger then tunning length, this pennalty
    // will become positive, which may help
    return -ChordSettings.instance().getRequiredBasicsGrade()
   ...
   * 
   */
  private float combinationHasGoodFingering(
      List<StringValue> stringValueCombination) {
    if (!isValidProcess()) {
      return 0;
    }
    ...
    for (int i = 0; i < this.tuning.length; i++) {
      ...
      openChord |= ChordSettings.instance().isEmptyStringChords()
          && positions[i] == 0;
    ...
    case 0:
      ...
      break;
    case 1:
      distanceGrade = ChordSettings.instance().getFingeringGrade()
          / (5 + maxCount);
      break;
    case 2:
      distanceGrade = ChordSettings.instance().getFingeringGrade()
      ...
      break;
    case 3:
      distanceGrade = -ChordSettings.instance().getFingeringGrade() / 10
          * maxCount;
      ...
      break;
    case 4:
      distanceGrade = -ChordSettings.instance().getFingeringGrade() / 4
          * maxCount;
      if (min <= 5)
        distanceGrade *= 1.8;
      break;
    ...
    // chord -35555)
    ArrayList[] fingers = { new ArrayList<Integer>(2),
    ...
    // - others are strings
    /*
     * for (int i=0; i<this.tuning.length; i++) LOG.debug(" "+positions[i]);
     * LOG.debug("");
     */

    // if chord is open, then we can have capo only in strings before open
    // string
    int lastZeroIndex = 0;
    ...
    /*
     * LOG.debug("Positions:"); for (int i=0; i<4; i++) { if
     * (fingers[i].size()>1)
     * LOG.debug("G"+(i+1)+"R"+((Integer)fingers[i].get(0))
     * .intValue()+"S"+((Integer)fingers[i].get(1)).intValue()+" "); }
     */

    if (finger > 4)
      finalGrade -= ChordSettings.instance().getFingeringGrade();
    else
      finalGrade += ChordSettings.instance().getFingeringGrade() * 0.1
          * (15 - 2 * finger);
  ...
  private float combinationHasSubsequentStrings(
      List<StringValue> stringValueCombination) {
    if (!isValidProcess()) {
      return 0;
    }
   ...
   * - has good chord semantics uses many basic tones, and all necessary tones
   * in their place<br>
   ...
   * 
   */
  private List<PriorityItem> determinePriority(
      List<List<StringValue>> allCombinations) {
    if (!isValidProcess()) {
      return null;
    }
    ...
    while (it.hasNext() && isValidProcess()) {
      ...
      priority += ChordSettings.instance().getManyStringsGrade()
          / 3
   ...
   * 
   */
  private void find(int stringTone, int stringIndex, int fret,
      List<StringValue> stringList) {
    if (!isValidProcess()) {
      return;
    }
    ...
    if (this.expandingNotes.length != 0) {
      for (int i = 0; i < this.expandingNotes.length; i++) {
        if ((stringTone + fret) % 12 == (this.chordTonic
            + this.expandingNotes[i] - 1) % 12) {
          if (!bassAlreadyIn && (stringTone + fret) % 12 == this.bassTonic)
            bassAlreadyIn = true;
          stringList.add(new StringValue(stringIndex, fret,
              (i < 2 ? this.ESSENTIAL_INDEX : this.NOT_ESSENTIAL_INDEX)));
        }
      }
    }
  ...
  protected void getChords(int[] tuning, int chordIndex, int alteration,
      int plusMinus, boolean add, int add5, int add9, int add11, int bassTonic,
      int chordTonic, boolean sharp) {
    ...
    this.chordName = new ChordNamingConvention().createChordName(
        this.chordTonic, this.chordIndex, this.alteration, plusMinus, add,
        add5, add9, add11, this.bassTonic, sharp);
   ...
   * 
   */
  private List<List<StringValue>> makeCombinations(
      List<List<StringValue>> potentialNotes) {
    if (!isValidProcess()) {
      return null;
    }
  ...
  private List<List<StringValue>> makePotentialNotes() {
    ...
    List<List<StringValue>> potentialNotes = new ArrayList<List<StringValue>>(
        this.tuning.length);
    ...
    for (int string = 0; string < this.tuning.length; string++) {
      ...
      if (ChordSettings.instance().getFindChordsMin() > 0
        ...
        find(this.tuning[string], string, 0, currentStringList);
      // if it's open chord but wanted to search from different minimal fret
   ...
   * 
   * @return structure of StringValue combinations : AL { AL(StringValue,SV,SV),
   *         AL(SV), AL(SV,SV),AL(SV,SV,SV,SV,SV,SV) }
   * 
   */
