...
public class TGMeasureImpl extends TGMeasure {
  public static final int ACCIDENTAL_FLAT_NOTES[] = new int[] { 0, 1, 1, 2, 2,
      3, 4, 4, 5, 5, 6, 6 };
  public static final boolean ACCIDENTAL_NOTES[] = new boolean[] { false, true,
      false, true, false, false, true, false, true, false, true, false };
  public static final int ACCIDENTAL_SHARP_NOTES[] = new int[] { 0, 0, 1, 1, 2,
      3, 3, 4, 4, 5, 5, 6 };
  /**
  ...
  public static final int SCORE_KEY_FLAT_POSITIONS[][] = new int[][] {
      ...
      new int[] { 4, 1, 5, 2, 6, 3, 7 }, new int[] { 6, 3, 7, 4, 8, 5, 9 }, };
  ...
  private void calculateBeats(ViewLayout layout) {
    ...
    for (int i = 0; i < countBeats(); i++) {
      ...
      if (chordEnabled && beat.getChord() != null) {
        if (previousChord != null) {
          int length = (int) (beat.getStart() - previousChord.getBeat()
              .getStart());
          minimumChordLength = (minimumChordLength > 0) ? Math.min(
              minimumChordLength, Math.abs(length)) : length;
        }
      ...
      for (int v = 0; v < TGBeat.MAX_VOICES; v++) {
        ...
        if (!voice.isEmpty()) {
          ...
          voice.reset();
          if (minDuration == null
              || voice.getDuration().getTime() <= minDuration.getTime()) {
          ...
          if (!voice.isRestVoice()) {
            ...
            if ((groups[v] == null)
                || !canJoin(layout.getSongManager(), voice, previousVoices[v])) {
            ...
            for (int v2 = 0; v2 < TGBeat.MAX_VOICES; v2++) {
              if (v2 != voice.getIndex()) {
                ...
                if (!voice2.isEmpty()
                    && voice2.getDuration().isEqual(voice.getDuration())) {
    ...
    if (!this.compactMode) {
      this.quarterSpacing = (minDuration != null) ? layout
          .getSpacingForQuarter(minDuration) : Math
          .round(DEFAULT_QUARTER_SPACING * layout.getScale());
      if (chordEnabled && minimumChordLength > 0) {
        int chordWidth = (layout.getChordFretIndexSpacing()
            + layout.getChordStringSpacing() + (getTrack().stringCount() * layout
  ...
  private int calculateKeySignatureSpacing(ViewLayout layout) {
    ...
    if (this.paintKeySignature) {
      if (this.getKeySignature() <= 7) {
        spacing += Math
            .round((6f * layout.getScale()) * this.getKeySignature());
      } else {
        spacing += Math.round((6f * layout.getScale())
            * (this.getKeySignature() - 7));
      }
      if (this.prevMeasure != null) {
        if (this.prevMeasure.getKeySignature() <= 7) {
          spacing += Math.round((6f * layout.getScale())
              * this.prevMeasure.getKeySignature());
        } else {
          spacing += Math.round((6f * layout.getScale())
              * (this.prevMeasure.getKeySignature() - 7));
        }
      }
    }
    return spacing;
  ...
  public void calculateMeasureChanges(ViewLayout layout) {
    ...
    this.prevMeasure = (layout.isFirstMeasure(this) ? null
        : (TGMeasureImpl) layout.getSongManager().getTrackManager()
            .getPrevMeasure(this));
    if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0) {
      ...
      if (this.prevMeasure == null
        ...
        this.getHeaderImpl().notifyKeySignatureSpacing(
            calculateKeySignatureSpacing(layout));
      }
    }
  }
  ...
  public void calculateWidth(ViewLayout layout) {
    if (this.compactMode) {
      ...
      double quartersInSignature = ((1.00 / this.getTimeSignature()
          .getDenominator().getValue()) * 4.00)
          * this.getTimeSignature().getNumerator();
  ...
  public boolean canJoin(TGSongManager manager, TGVoiceImpl b1, TGVoiceImpl b2) {
    ...
    long start = getStart();
    long start1 = (manager.getMeasureManager().getRealStart(this,
        b1.getBeat().getStart()) - start);
    long start2 = (manager.getMeasureManager().getRealStart(this,
        b2.getBeat().getStart()) - start);

    if (b1.getDuration().getValue() < TGDuration.EIGHTH
        || b2.getDuration().getValue() < TGDuration.EIGHTH) {
  ...
  private int getMaxQuarterSpacing(ViewLayout layout) {
    return (((layout.getStyle() & ViewLayout.DISPLAY_MULTITRACK) != 0) ? getHeaderImpl()
  ...
  public int getWidth(ViewLayout layout) {
    return ((layout.getStyle() & ViewLayout.DISPLAY_MULTITRACK) != 0 ? this
        .getHeaderImpl().getMaxWidth() : this.width);
  }
  ...
  /**
   * Retorna true si se esta reproduciendo y la posicion del player esta en este
   * compas.
   */
  public boolean isPlaying(ViewLayout layout) {
    return (getTrackImpl().hasCaret(layout) && TuxGuitar.instance()
        .getEditorCache().isPlaying(this));
  }

  private void makeBeat(ViewLayout layout, TGBeatImpl beat,
      TGBeatImpl previousBeat, boolean chordEnabled) {
    int minimumWidth = -1;
  ...
  private void makeVoice(ViewLayout layout, TGVoiceImpl voice,
      TGVoiceImpl previousVoice, TGBeatGroup group) {
   ...
   * Pinta la Clave
   */
  private void paintClef(ViewLayout layout, TGPainter painter, int fromX,
      int fromY) {
    // -----SCORE ------------------------------------//
  ...
  private void paintDivisions(ViewLayout layout, TGPainter painter) {
    ...
    if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
      ...
      if ((style & ViewLayout.DISPLAY_TABLATURE) != 0
          && (layout.isFirstMeasure(this) || isFirstOfLine())) {
        offsetY = (getPosY() + getTs().getPosition(
    ...
    if ((style & ViewLayout.DISPLAY_TABLATURE) != 0) {
      ...
      y2 = y1
          + ((getTrack().getStrings().size() - 1) * layout.getStringSpacing());
  ...
  private void paintDivisions(ViewLayout layout, TGPainter painter, int x1,
      int y1, int x2, int y2, int offsetY, boolean addInfo) {
    ...
    if (addInfo) {
      ...
      painter.drawString(number, getPosX() + Math.round(scale), (y1 - painter
          .getStringExtent(number).y)
          - Math.round(scale));
    }
    ...
    if (this.isRepeatOpen() || layout.isFirstMeasure(this)) {
      ...
      painter
          .lineTo(x1 + lineWidthBig + scale + lineWidthSmall, (y2 + offsetY));
      painter.closePath();
      ...
      if (this.isRepeatOpen()) {
        ...
        painter.addOval(x1 + xMove,
            y1 + ((y2 - y1) / 2) - (yMove + (size / 2)), size, size);
        painter.moveTo(x1 + xMove, y1 + ((y2 - y1) / 2) + (yMove - (size / 2)));
        painter.addOval(x1 + xMove,
            y1 + ((y2 - y1) / 2) + (yMove - (size / 2)), size, size);
        painter.closePath();
    ...
    if (this.getRepeatClose() > 0 || layout.isLastMeasure(this)) {
      ...
      painter.setAntialias(false);
      painter.setLineWidth(lineWidthSmall);
      painter.moveTo((x2 + getSpacing())
          - (lineWidthBig + scale + lineWidthSmall), y1);
      painter.lineTo((x2 + getSpacing())
          - (lineWidthBig + scale + lineWidthSmall), y2);
      painter.closePath();

      painter.initPath(TGPainter.PATH_DRAW | TGPainter.PATH_FILL);
      painter.setAntialias(false);
      painter.setLineWidth(lineWidthSmall);
      painter.addRectangle((x2 + getSpacing()) - lineWidthBig, y1,
          lineWidthBig, y2 - y1);
      painter.closePath();
      ...
      if (this.getRepeatClose() > 0) {
        ...
        painter.setLineWidth(lineWidthSmall);
        painter.initPath(TGPainter.PATH_FILL);
        painter.moveTo((x2 - xMove) + getSpacing(), y1 + ((y2 - y1) / 2)
            - (yMove + (size / 2)));
        painter.addOval((x2 - xMove) + getSpacing(), y1 + ((y2 - y1) / 2)
            - (yMove + (size / 2)), size, size);
        painter.moveTo((x2 - xMove) + getSpacing(), y1 + ((y2 - y1) / 2)
            + (yMove - (size / 2)));
        painter.addOval((x2 - xMove) + getSpacing(), y1 + ((y2 - y1) / 2)
            + (yMove - (size / 2)), size, size);
        ...
        if (addInfo) {
          ...
          painter.drawString(repetitions, x2 - numberSize.x + getSpacing()
              - size, (y1 - numberSize.y) - Math.round(scale));
        }
      }
    } else {
      painter.initPath();
   ...
   */
  private void paintKeySignature(ViewLayout layout, TGPainter painter,
      int fromX, int fromY) {
    if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0
        && this.paintKeySignature) {
      ...
      if (previousKey >= 1 && previousKey <= 7) {
        ...
        int naturalFrom = (currentKey >= 8 && currentKey <= 14) ? currentKey
            : 7;
        for (int i = naturalFrom; i < previousKey; i++) {
  ...
  private void paintLoopMarker(ViewLayout layout, TGPainter painter) {
    if (layout.hasLoopMarker(this)) {
      int size = Math.round(layout.getLoopMarkerSpacing()
          - (1f * layout.getScale()));
      ...
      if (pm.getLoopSHeader() == getHeader().getNumber()) {
        ...
        int y = (getPosY() + getTs().getPosition(
            TGTrackSpacing.POSITION_LOOP_MARKER));
      ...
      if (pm.getLoopEHeader() == getHeader().getNumber()) {
        ...
        float y = (getPosY() + getTs().getPosition(
            TGTrackSpacing.POSITION_LOOP_MARKER));
  ...
  public void paintMeasure(ViewLayout layout, TGPainter painter) {
    ...
    if (shouldRepaintBuffer() || !bufferEnabled) {
      ...
      if (bufferEnabled) {
        getBuffer().createBuffer(getWidth(layout) + getSpacing(),
            getTs().getSize(), layout.getResources().getBackgroundColor());
        bufferPainter = getBuffer().getPainter();
      }
      layout.paintLines(getTrackImpl(), getTs(), bufferPainter, x, y,
          getWidth(layout) + getSpacing());
    ...
    if (bufferEnabled) {
      ...
      getBuffer().paintBuffer(painter, getPosX(), getPosY(),
          getTs().getPosition(TGTrackSpacing.POSITION_BUFFER_SEPARATOR));
    }
  ...
  private void paintPlayMode(ViewLayout layout, TGPainter painter) {
    if (layout.isPlayModeEnabled() && isPlaying(layout)) {
      ...
      if ((style & (ViewLayout.DISPLAY_SCORE | ViewLayout.DISPLAY_TABLATURE)) == (ViewLayout.DISPLAY_SCORE | ViewLayout.DISPLAY_TABLATURE)) {
        y1 += (getTs().getPosition(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) - layout
            .getScoreLineSpacing());
        y2 += (getTs().getPosition(TGTrackSpacing.POSITION_TABLATURE)
            + getTrackImpl().getTabHeight() + layout.getStringSpacing());
      } else if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
        y1 += (getTs().getPosition(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) - layout
            .getScoreLineSpacing());
        y2 += (getTs().getPosition(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) + (layout
            .getScoreLineSpacing() * 5));
      } else if ((style & ViewLayout.DISPLAY_TABLATURE) != 0) {
        y1 += (getTs().getPosition(TGTrackSpacing.POSITION_TABLATURE) - layout
            .getStringSpacing());
        y2 += (getTs().getPosition(TGTrackSpacing.POSITION_TABLATURE)
            + getTrackImpl().getTabHeight() + layout.getStringSpacing());
      }
      ...
      painter.initPath();
      painter.setAntialias(false);
      painter.addRectangle(getPosX() + (5f * scale), y1, width - (10f * scale),
          (y2 - y1));
      painter.closePath();
  ...
  public void paintRepeatEnding(ViewLayout layout, TGPainter painter) {
    if (getHeader().getRepeatAlternative() > 0) {
      ...
      float y1 = (getPosY() + getTs().getPosition(
          TGTrackSpacing.POSITION_REPEAT_ENDING));
      float y2 = (y1 + (layout.getRepeatEndingSpacing() * 0.75f));
      String string = new String();
      for (int i = 0; i < 8; i++) {
        if ((getHeader().getRepeatAlternative() & (1 << i)) != 0) {
          string += ((string.length() > 0) ? (", ") + Integer.toString(i + 1)
      ...
      painter.closePath();
      painter.drawString(string, Math.round(x1 + (5.0f * scale)), Math.round(y1
          + (2.0f * scale)));
    }
  }
  ...
  private void paintTempo(ViewLayout layout, TGPainter painter) {
    if (this.getHeaderImpl().shouldPaintTempo()) {
      ...
      int y = getPosY();
      int lineSpacing = (Math.max(layout.getScoreLineSpacing(), layout
          .getStringSpacing()));
      int style = layout.getStyle();
      if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
      ...
      int fontY = Math.round(y - painter.getStringExtent(value).y
          - (1.0f * layout.getScale()));
    ...
    for (final TGBeat beat : getBeats()) {
      if (beat.isTextBeat()) {
        ...
        text.paint(layout, painter, (getPosX() + getHeaderImpl()
            .getLeftSpacing(layout)), getPosY());
      }
    }
  }

  private void paintTimeSignature(ViewLayout layout, TGPainter painter,
      int fromX, int fromY) {
    if (this.getHeaderImpl().shouldPaintTimeSignature()) {
      ...
      int x = (getClefSpacing(layout) + getKeySignatureSpacing(layout)
          + getHeaderImpl().getLeftSpacing(layout) + leftSpacing);
      String numerator = Integer.toString(getTimeSignature().getNumerator());
      String denominator = Integer.toString(getTimeSignature().getDenominator()
          .getValue());
      if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
        int y = getTs().getPosition(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES);
        int y1 = (int) (y - (3f * scale));
        int y2 = (int) (((y + (layout.getScoreLineSpacing() * 4)) - painter
            .getStringExtent(denominator).y) + (3f * scale));
        ...
        int y1 = (y - move);
        int y2 = ((y + getTrackImpl().getTabHeight()) - painter
            .getStringExtent(denominator).y)
            + move;
        painter.drawString(numerator, fromX + x, fromY + y1, true);
        painter.drawString(denominator, fromX + x, fromY + y2, true);
      }
  ...
  private void paintTripletFeel(ViewLayout layout, TGPainter painter) {
    if (this.getHeaderImpl().shouldPaintTripletFeel()) {
      float scale = (5f * layout.getScale());
      int x = (getPosX() + getHeaderImpl().getLeftSpacing(layout) + getHeaderImpl()
          .getTempoSpacing(layout));
      int y = (getPosY());
      int lineSpacing = (Math.max(layout.getScoreLineSpacing(), layout
          .getStringSpacing()));
      int style = layout.getStyle();
      ...
      int y2 = y
          - (Math.round((1.0f * scale) + (2.5f * scale) + (1.0f * scale)) + 2);

      if (getTripletFeel() == TGMeasureHeader.TRIPLET_FEEL_NONE
        ...
        if (previous == TGMeasureHeader.TRIPLET_FEEL_EIGHTH) {
          if (layout.isBufferEnabled()) {
            ...
            painter.drawImage(layout.getResources().getTripletFeelNone8(), x2,
                y1);
          } else {
          ...
          if (layout.isBufferEnabled()) {
            ...
            painter.drawImage(layout.getResources().getTripletFeelNone16(), x2,
                y1);
          } else {
        ...
        if (layout.isBufferEnabled()) {
          painter
              .drawImage(layout.getResources().getTripletFeelNone8(), x1, y1);
        ...
        if (layout.isBufferEnabled()) {
          painter.drawImage(layout.getResources().getTripletFeelNone16(), x1,
              y1);
  ...
  public void registerSpacing(ViewLayout layout, TGTrackSpacing ts) {
    if (layout.hasLoopMarker(this)) {
      ts.setSize(TGTrackSpacing.POSITION_LOOP_MARKER, layout
          .getLoopMarkerSpacing());
    }
    if (this.hasMarker()) {
    ...
    if (this.chord) {
      ts
          .setSize(TGTrackSpacing.POSITION_CHORD, layout
    ...
    if (this.getHeader().getRepeatAlternative() > 0) {
      ts.setSize(TGTrackSpacing.POSITION_REPEAT_ENDING, layout
          .getRepeatEndingSpacing());
    }
    if (this.division) {
      ts.setSize(TGTrackSpacing.POSITION_DIVISION_TYPE, layout
          .getDivisionTypeSpacing());
    }
  ...
  public void setTs(TGTrackSpacing ts) {
    if (getTs() == null) {
      setBufferCreated(false);
    } else if (getTs().getPosition(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) != ts
        .getPosition(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES)) {
      setBufferCreated(false);
    } else if (getTs().getPosition(TGTrackSpacing.POSITION_TABLATURE) != ts
        .getPosition(TGTrackSpacing.POSITION_TABLATURE)) {
      setBufferCreated(false);
    } else if (getTs().getPosition(TGTrackSpacing.POSITION_EFFECTS) != ts
        .getPosition(TGTrackSpacing.POSITION_EFFECTS)) {
      setBufferCreated(false);
    }
    this.ts = ts;
  }
  ...
  private void updateComponents(ViewLayout layout) {
    ...
    for (int i = 0; i < countBeats(); i++) {
      ...
      if (this.compactMode) {
        ...
        int x1 = (spacing + TablatureUtil.getStartPosition(this, beat
            .getStart(), quarterWidth));
        int minimumWidth = -1;
        for (int v = 0; v < beat.countVoices(); v++) {
          TGVoiceImpl voice = beat.getVoiceImpl(v);
          if (!voice.isEmpty()) {
            int x2 = (spacing + TablatureUtil.getStartPosition(this, beat
                .getStart()
                + voice.getDuration().getTime(), quarterWidth));
            int width = (x2 - x1);
      ...
      for (int v = 0; v < beat.countVoices(); v++) {
        ...
        if (!voice.isEmpty()) {
          ...
          if (!this.division
              && !voice.getDuration().getDivision().isEqual(
                  TGDivisionType.NORMAL)) {
            this.division = true;
          }
          if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) == 0
              || (voice.isRestVoice() && !voice.isHiddenSilence())) {
