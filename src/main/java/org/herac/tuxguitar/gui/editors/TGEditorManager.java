...
public class TGEditorManager {
  ...
  public void doRedraw(int type) {
    for (int i = 0; i < this.redrawListeners.size(); i++) {
      TGRedrawListener listener = (TGRedrawListener) this.redrawListeners
          .get(i);
  ...
  public void doUpdate(int type) {
    for (int i = 0; i < this.updateListeners.size(); i++) {
      TGUpdateListener listener = (TGUpdateListener) this.updateListeners
          .get(i);
  ...
  public void hideExternalBeat() {
    for (int i = 0; i < this.beatViewerListeners.size(); i++) {
      TGExternalBeatViewerListener listener = (TGExternalBeatViewerListener) this.beatViewerListeners
          .get(i);
