...
public class ChordCustomList extends Composite {
  ...
  public ChordCustomList(ChordDialog dialog, Composite parent, int style,
      int height) {
    super(parent, style);
  ...
  protected void addCustomChord() {
    ...
    if (chord != null) {
      ...
      if (name != null) {
        if (name.length() == 0) {
          MessageDialog.errorMessage(getShell(), TuxGuitar
              .getProperty("chord.custom.name-empty-error"));
          return;
        }
        if (TuxGuitar.instance().getCustomChordManager()
            .existOtherEqualCustomChord(name, -1)) {
          MessageDialog.errorMessage(getShell(), TuxGuitar
              .getProperty("chord.custom.name-exist-error"));
          return;
        }
        chord.setName(name);
  ...
  private void loadChords() {
    ...
    for (int i = 0; i < TuxGuitar.instance().getCustomChordManager()
        .countChords(); i++) {
    ...
    if (selectionIndex >= 0 && selectionIndex < this.chords.getItemCount()) {
      this.chords.select(selectionIndex);
        ...
        && (selectionIndex - 1) < this.chords.getItemCount()) {
  ...
  protected void removeCustomChord(int index) {
    if (index >= 0
        && index < TuxGuitar.instance().getCustomChordManager().countChords()) {
  ...
  protected void renameCustomChord(int index) {
    TGChord chord = TuxGuitar.instance().getCustomChordManager()
        .getChord(index);
    if (chord != null) {
      ...
      if (name != null) {
        ...
        if (TuxGuitar.instance().getCustomChordManager()
            .existOtherEqualCustomChord(name, index)) {
          MessageDialog.errorMessage(getShell(), TuxGuitar
              .getProperty("chord.custom.name-exist-error"));
          return;
        }
