...
public class Piano extends Composite {
  ...
  private static final boolean TYPE_NOTES[] = new boolean[] { true, false,
    ...
    if (voice != null) {
      ...
      if (beat != null) {
        ...
        if (measure != null) {
          ...
          if (track != null) {
            return (note.getValue() + track.getString(note.getString())
                .getValue());
          }
        }
      }
  ...
  public void loadIcons() {
    this.getShell()
        .setImage(TuxGuitar.instance().getIconManager().getAppIcon());
    this.settings.setImage(TuxGuitar.instance().getIconManager().getSettings());
  ...
  private void makePiano() {
    ...
    this.pianoComposite.setLayoutData(new GridData(
  ...
  private Image makePianoImage() {
    ...
    painter.addRectangle(x, y, (NATURAL_WIDTH * (MAX_OCTAVES * NATURAL_NOTES)),
        NATURAL_HEIGHT);
    painter.closePath();
    for (int i = 0; i < (MAX_OCTAVES * TYPE_NOTES.length); i++) {
      ...
      if (TYPE_NOTES[i % TYPE_NOTES.length]) {
        ...
        painter.initPath(TGPainter.PATH_FILL);
        painter.setAntialias(false);
        painter.addRectangle(x - (SHARP_WIDTH / 2), y, SHARP_WIDTH,
            SHARP_HEIGHT);
        painter.closePath();
      }
    }
    ...
    for (int i = 0; i < (MAX_OCTAVES * TYPE_NOTES.length); i++) {
      ...
      if (i == value) {
        if (TYPE_NOTES[i % TYPE_NOTES.length]) {
          ...
          painter.addRectangle(x + 1, (y + SHARP_HEIGHT) + 1,
              NATURAL_WIDTH - 1, (NATURAL_HEIGHT - SHARP_HEIGHT) - 1);
          painter.closePath();
        } else {
          painter.initPath(TGPainter.PATH_FILL);
          painter.setAntialias(false);
    ...
    for (int i = 0; i < (MAX_OCTAVES * TYPE_NOTES.length); i++) {
      ...
      if (TuxGuitar.instance().getScaleManager().getScale().getNote(i)) {
        if (TYPE_NOTES[i % TYPE_NOTES.length]) {
          ...
          painter.initPath(TGPainter.PATH_FILL);
          painter.setAntialias(false);
          painter.addRectangle((x + 1 + (((NATURAL_WIDTH - size) / 2))),
              (NATURAL_HEIGHT - size - (((NATURAL_WIDTH - size) / 2))), size,
              size);
          painter.closePath();
        } else {
          painter.initPath(TGPainter.PATH_FILL);
          painter.setAntialias(false);
          painter.addRectangle(posX + 1, SHARP_HEIGHT - SHARP_WIDTH + 1,
              SHARP_WIDTH - 2, SHARP_WIDTH - 2);
          painter.closePath();
        }
      }
