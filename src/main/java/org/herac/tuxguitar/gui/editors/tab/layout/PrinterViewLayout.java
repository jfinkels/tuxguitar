...
public class PrinterViewLayout extends ViewLayout {
  ...
  public Color getDarkColor(Color color) {
    return (this.styles.isBlackAndWhite() ? getResources().getColorBlack()
        : color);
  }
  ...
  private TempLine getTempLines(TGTrack track, int fromIndex, TGTrackSpacing ts) {
    ...
    for (int measureIdx = fromIndex; measureIdx < measureCount; measureIdx++) {
      ...
      if (measure.getNumber() >= this.styles.getFromMeasure()
          && measure.getNumber() <= this.styles.getToMeasure()) {

        // verifico si tengo que bajar de linea
        if ((line.tempWith + measure.getWidth(this)) >= getMaxWidth()
            && !line.measures.isEmpty()) {
          line.fullLine = true;
          return line;
        }
        line.tempWith += measure.getWidth(this);
        line.maxY = (measure.getMaxY() > line.maxY) ? measure.getMaxY()
            : line.maxY;
        line.minY = (measure.getMinY() < line.minY) ? measure.getMinY()
            : line.minY;
  ...
  protected void init(float scale) {
    ...
    this.setFirstMeasureSpacing(getScaledValue(scale,
        DEFAULT_HORIZONTAL_SPACING));
    this.setMinBufferSeparator(getScaledValue(scale,
        DEFAULT_MIN_BUFFER_SEPARATOR));
    ...
    this.setScoreLineSpacing(getScaledValue(scale, DEFAULT_SCORE_LINE_SPACING,
        4));
    this
        .setScoreSpacing(((getScoreLineSpacing() * 4) + getMinScoreTabSpacing()));
    this
        .setFirstTrackSpacing(getScaledValue(scale, DEFAULT_FIRST_TRACK_SPACING));
    ...
    String fontName = (fd != null && fd.length > 0 ? fd[0].getName()
        : new String());
    this.songNameFont = new Font(getTablature().getDisplay(), fontName, Math
        .round(16.0f * getFontScale()), SWT.BOLD | SWT.CENTER);
    this.trackNameFont = new Font(getTablature().getDisplay(), fontName, Math
        .round(8.0f * getFontScale()), SWT.BOLD | SWT.CENTER);
    this.songAuthorFont = new Font(getTablature().getDisplay(), fontName, Math
        .round(8.0f * getFontScale()), SWT.BOLD | SWT.CENTER);
  }
  ...
  private void makeDocument() {
    ...
    paintSong(this.document.getPainter(), null, this.document.getBounds().x,
        (this.document.getBounds().y + Math.round(80.0f * getFontScale())));
  ...
  private void paintFooter(TGPainter painter) {
    if (this.document.isPaintable(this.page)) {
      ...
      painter.drawString(pageNumber, (x + getRight(painter, pageNumber)),
          (y + getBottom(painter, pageNumber)));
    }
  }
  ...
  public void paintHeader(TGPainter painter) {
    if (this.document.isPaintable(this.page)) {
      ...
      String trackName = "("
          + getSongManager().getTrack(this.styles.getTrackNumber()).getName()
          + ")";
      ...
      painter.drawString(trackName, (x + getCenter(painter, trackName)),
          (y + Math.round(30.0f * getFontScale())));
      painter.setFont(this.songAuthorFont);
      painter.drawString(songAuthor, (x + getRight(painter, songAuthor)),
          (y + Math.round(50.0f * getFontScale())));
    }
  }

  public void paintLine(TGTrackImpl track, TempLine line, TGPainter painter,
      int fromX, int fromY, TGTrackSpacing ts) {
    if (this.document.isPaintable(this.page)) {
      int posX = fromX;
      int posY = fromY;
      int width = 0;
      ...
      for (int i = 0; i < line.measures.size(); i++) {
        ...
        ((TGLyricImpl) track.getLyrics()).paintCurrentNoteBeats(painter, this,
            currMeasure, posX, posY);

        int measureWidth = (currMeasure.getWidth(this) + currMeasure
            .getSpacing());
        posX += measureWidth;
        width += measureWidth;
      }
  ...
  public void paintSong(TGPainter painter, Rectangle clientArea, int fromX,
      int fromY) {
    this.setWidth(0);
    this.setHeight(0);
    ...
    TGTrackImpl track = (TGTrackImpl) getSongManager().getTrack(
        this.styles.getTrackNumber());
    ...
    while (!line.measures.isEmpty()) {
      ...
      ts.setSize(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES,
      ...
      if ((style & DISPLAY_SCORE) != 0) {
        ...
        if (line.maxY + getMinScoreTabSpacing() > getScoreSpacing()) {
          ts.setSize(TGTrackSpacing.POSITION_SCORE_DOWN_LINES,
              (line.maxY - (getScoreLineSpacing() * 4)));
        }
      }
      if ((style & DISPLAY_TABLATURE) != 0) {
        ts.setSize(TGTrackSpacing.POSITION_TABLATURE_TOP_SEPARATOR,
            ((style & DISPLAY_SCORE) != 0 ? getMinScoreTabSpacing() : Math.max(
                Math.abs(line.minY), getStringSpacing())));
        ts.setSize(TGTrackSpacing.POSITION_TABLATURE,
            ((style & DISPLAY_SCORE) != 0 ? track.getTabHeight()
                + getStringSpacing() + 1 : Math.max(line.maxY, track
                .getTabHeight()
                + getStringSpacing() + 1)));
      }
  ...
  public void setTempoStyle(TGPainter painter, boolean fontStyle) {
    painter.setFont(getResources().getPrinterDefaultFont());
    painter.setForeground(getResources().getColorBlack());
    painter.setBackground((fontStyle ? getResources().getColorWhite()
