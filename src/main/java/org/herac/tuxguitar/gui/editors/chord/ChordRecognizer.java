...
public class ChordRecognizer extends Composite {
  ...
  void findChordLogic(Proposal current) {
    ...
    /*
     * if (!found[3]) current.unusualGrade-=50;
     */
  ...
  protected String getChordName(int[] param, boolean sharp) {
    return new ChordNamingConvention().createChordName(param[TONIC_INDEX],
        param[CHORD_INDEX], param[ALTERATION_INDEX], param[PLUSMINUS_INDEX],
        param[ADDCHK_INDEX] != 0, param[I5_INDEX], param[I9_INDEX],
        param[I11_INDEX], param[BASS_INDEX], sharp);
  }
   ...
   */
  protected int[] makeProposals(final long processId, TGChord chord,
      final boolean sharp) {
    ...
    for (int tonic = 0; tonic < 12; tonic++) {
      ...
      for (int chordIndex = 0; chordIndex < ChordDatabase.length(); chordIndex++) {
        ...
        currentProp.unusualGrade -= (chordIndex != ChordDatabase.length()
            && chordIndex != 4 ? 2 * chordIndex : 0);

        // ChordDatabase.ChordInfo info =
        ...
        if (foundNote) {
          ...
          int foundNotesCount = currentProp.missingNotes.length
              - currentProp.missingCount;

          // it is worth if it is missing 1 essential note and/or fifth
          if (!info.getName().startsWith("dim")
              && !info.getName().startsWith("aug"))
            if (!currentProp.isFound(tonic + 8 - 1)) {

              // hmmm. maybe it's altered fifth? Create a branch for it.
              if (currentProp.isNeeded(tonic + 7 - 1)
                  || currentProp.isNeeded(tonic + 9 - 1)) {
    ...
    for (final Proposal current : allProposals) {
      ...
      for (int i = chord.getStrings().length - 1; i >= 0; i--) {
        if (chord.getStrings()[i] != -1) {
          ...
          if (current.params[BASS_INDEX] == (tuning[tuning.length - 1 - i] + chord
              .getStrings()[i]) % 12)
      ...
      if (current.missingCount > 0 && current.params[CHORD_INDEX] <= 11) {
        ...
        int seventh;
        if (current.params[CHORD_INDEX] == 2)
          seventh = current.params[TONIC_INDEX] + 12 - 1; // plain 7
        else
          seventh = current.params[TONIC_INDEX] + 11 - 1; // b7
        ...
        for (int plusminus = 0; plusminus <= 2; plusminus++) {
          for (int i = 2; i >= 0; i--)
            // 13, 11, 9
            if (current.isNeeded(current.params[TONIC_INDEX]
                + getAddNote(i, plusminus))
                && !current.filled[i]) {
              current.filled[i] = true;
              current.plusminusValue[i] = plusminus;
              if (plusminus != 0)
                current.unusualGrade -= 15;
              current.foundNote(current.params[TONIC_INDEX]
                  + getAddNote(i, plusminus));
            }
    ...
    // cut the search
    unsortedProposals = unsortedProposals.subList(0, (cut > 0
        && cut < unsortedProposals.size() ? cut : unsortedProposals.size()));
              ...
              new TGSynchronizer.TGRunnable() {
                public void run() {
                  if (!getDialog().isDisposed() && isValidProcess(processId)) {
                    addProposal(current.params, getChordName(current.params,
                        sharp)
                        + " ("
                        + Math.round(100 + current.dontHaveGrade * 7 / 10)
                        + "%)");
                  }
                }
              });
  ...
  /**
   * - Recognizes the chord string - Fills the component's list with alternative
   * names - Sets all the ChordSelector fields into recognized chord (tonic,
   * bass, chord, alterations) - Makes the alternatives and puts them into
   * ChordList - Writes the chord formula into appropriate label
   * 
   * @param chord
   *          chord structure (frets, strings)
   * @param redecorate
   *          is the Chord Editor in editing mode, or it is just changed by
   *          ChordSelector
   */

  public void recognize(final TGChord chord, final boolean redecorate,
      final boolean setChordName) {

    final long processId = (++this.runningProcess);
    final boolean sharp = this.dialog.getSelector().getSharpButton()
        .getSelection();
    ...
    new Thread(new Runnable() {
      public void run() {
        if (!getDialog().isDisposed() && isValidProcess(processId)) {
          ...
          // Sets all the ChordSelector fields into recognized chord (tonic,
          // bass, chord, alterations)
  ...
  protected void redecorate(int params[]) {
    this.dialog.getSelector().adjustWidgets(params[TONIC_INDEX],
        params[CHORD_INDEX], params[ALTERATION_INDEX], params[BASS_INDEX],
        params[PLUSMINUS_INDEX], params[ADDCHK_INDEX], params[I5_INDEX],
        params[I9_INDEX], params[I11_INDEX]);
  }
