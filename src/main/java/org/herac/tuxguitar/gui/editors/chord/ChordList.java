 ...
 * @author julian
 * 
 *         Component that shows the list of (alternative) chords - bottom of the
 *         screen
 */
public class ChordList extends Composite {
  ...
  protected Font getFont(GC painter) {
    if (this.font == null || this.font.isDisposed()) {
      ...
      if (datas.length > 0) {
        this.font = new Font(getDisplay(), datas[0].getName(), Math.min(7,
            datas[0].getHeight()), SWT.BOLD);
      }
    }
    return this.font;
  }
  ...
  protected int getTrackString(int number) {
    TGString string = ChordList.this.beat.getMeasure().getTrack().getString(
        number);
    return string.getValue();
  }

  private void init() {
    this.composite = new Composite(this, SWT.BORDER | SWT.V_SCROLL
        | SWT.DOUBLE_BUFFERED);
