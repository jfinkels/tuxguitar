...
public class ChordDialog {
  ...
  protected TGChord findCurrentChord(TGMeasure measure, long start) {
    ...
    if (chord == null) {
      ...
      if (!notes.isEmpty()) {
        ...
        for (final TGNote note : notes) {
          if (maxValue < 0 || maxValue < note.getValue()) {
            maxValue = note.getValue();
          }
          if (note.getValue() > 0
              && (minValue < 0 || minValue > note.getValue())) {
  ...
  public GridLayout gridLayout(int numColumns, boolean makeColumnsEqualWidth,
      int marginWidth, int marginHeight) {
    GridLayout layout = new GridLayout();
    layout.numColumns = numColumns;
    layout.makeColumnsEqualWidth = makeColumnsEqualWidth;
    layout.marginWidth = (marginWidth >= 0) ? marginWidth : layout.marginWidth;
    layout.marginHeight = (marginHeight >= 0) ? marginHeight
        : layout.marginHeight;
    return layout;
  }
  ...
  public int open(Shell shell, final TGMeasureImpl measure, TGBeat beat,
      long start) {
    ...
    // ---------------EDITOR--------------------------------
    this.editor = new ChordEditor(this, topComposite, DEFAULT_STYLE,
        (short) tuning.length);
    this.editor.pack();
    ...
    // ---------------CUSTOM CHORDS---------------------------------
    new ChordCustomList(this, topComposite, DEFAULT_STYLE, Math.max(
        this.selector.getBounds().height, this.editor.getBounds().height));
