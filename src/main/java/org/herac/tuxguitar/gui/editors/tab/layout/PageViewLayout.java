...
public class PageViewLayout extends ViewLayout {
  ...
  private static final int STATIC_WIDTH = TuxGuitar.instance().getConfig()
      .getIntConfigValue(TGConfigKeys.LAYOUT_PAGE_FORCE_WIDTH, 0);
  ...
  public void paintLine(TGTrackImpl track, TempLine line, TGPainter painter,
      int fromX, int fromY, TGTrackSpacing ts, Rectangle clientArea) {
    int posX = (this.marginLeft + fromX);
    int posY = fromY;
    int width = this.marginLeft;

    // verifico si esta en el area de cliente
    boolean isAtY = (posY + ts.getSize() > clientArea.y && posY < clientArea.y
        + clientArea.height + 80);
    ...
    for (int i = 0; i < line.measures.size(); i++) {
      ...
      boolean isAtX = (posX + measureWidth > clientArea.x && posX < clientArea.x
          + clientArea.width);
      if (isAtX && isAtY) {
  ...
  public void paintSong(TGPainter painter, Rectangle clientArea, int fromX,
    ...
    int style = getStyle();
    int number = ((style & ViewLayout.DISPLAY_MULTITRACK) == 0 ? getTablature()
    ...
    while (measureCount > nextMeasureIndex) {
      ...
      while (tracks.hasNext()) {
        ...
        if (number < 0 || track.getNumber() == number) {
          ...
          ts
              .setSize(TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES,
                  ((style & DISPLAY_SCORE) != 0 ? ((getScoreLineSpacing() * 5))
                      : 0));
          ...
          if ((style & DISPLAY_SCORE) != 0) {
            ts.setSize(TGTrackSpacing.POSITION_SCORE_UP_LINES, Math
                .abs(line.minY));
          ...
          if ((style & DISPLAY_TABLATURE) != 0) {
            ts.setSize(TGTrackSpacing.POSITION_TABLATURE_TOP_SEPARATOR,
                ((style & DISPLAY_SCORE) != 0 ? getMinScoreTabSpacing() : Math
                    .max(Math.abs(line.minY), getStringSpacing())));
          ...
          if ((emptyWith - 20) > 0 && (line.lastIndex + 1) >= measureCount) {
            if (emptyX < (clientArea.x + clientArea.width)) {
              ...
              emptyWith = (emptyWith > clientArea.width ? clientArea.width
                  : emptyWith);
