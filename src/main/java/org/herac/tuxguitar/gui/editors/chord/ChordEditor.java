...
public class ChordEditor extends Composite {
  ...
  public ChordEditor(ChordDialog dialog, Composite parent, int style,
      short maxStrings) {
    this(parent, style);
    this.dialog = dialog;
    ...
    if (string >= 1 && string <= this.maxStrings) {
      ...
      if (realValue == 0) {
        ...
        if (realValue > 0 && realValue <= TGChordImpl.MAX_FRETS) {
          this.addPoint(new Point(this.strings[this.maxStrings - string],
              this.frets[realValue - 1]));
        }
      }
      // INNECESARY CODE
      ...
      // this.chordName.setText(this.dialog.getRecognizer().recognize(getChord(),
      // redecorate));
    }
  }
  ...
  public void init(short maxStrings) {
    ...
    this.composite = new Composite(composite, SWT.BORDER | SWT.V_SCROLL
        | SWT.DOUBLE_BUFFERED);
    ...
    Label formulaLabel = new Label(nameComposite, SWT.SEPARATOR
        | SWT.HORIZONTAL);
    ...
    this.composite.setBackground(this.getDisplay().getSystemColor(
        SWT.COLOR_WHITE));
    ...
    this.composite.getVerticalBar().setMaximum(
        ((MAX_FRET + MIN_FRET) - (TGChordImpl.MAX_FRETS - 1) + 1));
  ...
  private GridData makeCompositeData() {
    ...
    data.minimumWidth = (getWidth() + (STRING_SPACING * 2) + this.composite
  ...
  protected void paintEditor(TGPainter painter) {
    ...
    painter.drawString(Integer.toString(getFret()), FRET_SPACING - 25,
        STRING_SPACING);
    ...
    new Thread(new Runnable() {
      public void run() {
        ...
        for (int i = 0; i < stringCount; i++) {
          ...
          if (value != -1) {
            beat[next][0] = getCurrentTrack().getOffset()
                + getCurrentTrack().getString(string).getValue() + value;
        ...
        TuxGuitar.instance().getPlayer().playBeat(ch.getChannel(),
            ch.getInstrument(), ch.getVolume(), ch.getBalance(),
            ch.getChorus(), ch.getReverb(), ch.getPhaser(), ch.getTremolo(),
            beat, 200, 200);
      }
    }).start();
  }
  ...
  public void setChord(TGChord chord) {
    if (chord != null) {
      ...
      // this.getChordName().setText(chord.getName() != null ? chord.getName() :
      // this.dialog.getRecognizer().recognize(getChord(),true) );

      String name = chord.getName();

      this.dialog.getRecognizer().recognize(getChord(), (name == null),
          (name == null));
