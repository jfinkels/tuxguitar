 ...
 *         Chord theory according to <a
 *         href="http://www.jazzguitar.be/quick_crd_ref.html"
 *         >http://www.jazzguitar.be/quick_crd_ref.html</a>.
 */
public class ChordSelector extends Composite {
  ...
  public ChordSelector(ChordDialog dialog, Composite parent, int style,
      int[] tuning) {
    super(parent, style);
  ...
  protected void adjustWidgetAvailability() {
    String chordName = ChordDatabase.get(getChordList().getSelectionIndex())
        .getName();
    if (chordName.equals("dim") || chordName.equals("dim7")
        || chordName.equals("aug") || chordName.equals("5")) {
    ...
    if (this.alterationList.isEnabled()) {
      ...
      updateWidget(this._9List, (currentIndex >= 2 && !this.addCheck
          .getSelection()));
      updateWidget(this._11List, (currentIndex >= 3 && !this.addCheck
          .getSelection()));
    }
  }

  /**
   * Sets all the widgets' fields into recognized chord (tonic, bass, chord,
   * alterations)
   */
  public void adjustWidgets(int tonic, int chordBasic, int alteration,
      int bass, int plusMinus, int addBoolean, int index5, int index9,
      int index11) {
  ...
  public void init() {
    ...
    customizeButton.setLayoutData(new GridData(SWT.FILL, SWT.BOTTOM, true,
        false));
    ...
    /*
     * Iterator chordInfo =
     * ChordCreatorUtil.getChordData().getChords().iterator();
     * while(chordInfo.hasNext()) { this.chordList.add(
     * ((ChordDatabase.ChordInfo)chordInfo.next()).getName() ); }
     */
    ...
    this.addCheck.addSelectionListener(new SelectionAdapter() {
      public void widgetSelected(SelectionEvent arg0) {
        if (getDialog().getEditor() != null && getDialog().getList() != null) {
          ...
          ChordSelector.this.adjustWidgetAvailability();
          /*
           * if (getAddCheck().getSelection()) { updateWidget(get_9List(),
           * false); updateWidget(get_11List(), false); }
           */
          if (ChordSelector.this.getRefresh()) {
            showChord();
  ...
  protected void initChordWidgets() {
    ...
    alterationComposite.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    ...
    this.alterationList.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    this.plusMinusList = new List(firstComposite, SWT.BORDER);
    this.plusMinusList.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
  ...
  protected void showChord() {
    ...
    ChordCreatorListener listener = new ChordCreatorListener() {
      public void notifyChords(final ChordCreatorUtil instance,
          final java.util.List<TGChord> chords) {
        try {
              ...
              new TGSynchronizer.TGRunnable() {
                public void run() {
                  if (instance.isValidProcess() && !getDialog().isDisposed()) {
                    ...
                    TuxGuitar.instance().loadCursor(getShell(),
                        SWT.CURSOR_ARROW);
                  }
                }
              });
    ...
    ChordCreatorUtil.getChords(listener, this.tuning, this.chordList
        .getSelectionIndex(), this.alterationList.getSelectionIndex(),
        this.plusMinusList.getSelectionIndex(), this.addCheck.getSelection(),
        this._5List.getSelectionIndex(), this._9List.getSelectionIndex(),
