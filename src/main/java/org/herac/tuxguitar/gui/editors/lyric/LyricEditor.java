...
public class LyricEditor implements TGUpdateListener, IconLoader,
    LanguageLoader {
  ...
  protected static final KeyBindingAction KB_ACTIONS[] = new KeyBindingAction[] {
      new KeyBindingAction(UndoAction.NAME, new KeyBinding(122,
          KeyBindingConstants.CONTROL)),
      new KeyBindingAction(RedoAction.NAME, new KeyBinding(121,
          KeyBindingConstants.CONTROL)), };
  ...
  public void doUpdate(int type) {
    if (type == TGUpdateListener.SELECTION) {
      this.updateItems();
        ...
        || type == TGUpdateListener.SONG_LOADED) {
      this.update();
    }
  }
  ...
  private void loadLyricText(Composite parent) {
    ...
    this.text = new Text(composite, SWT.BORDER | SWT.MULTI | SWT.WRAP
        | SWT.V_SCROLL);
    ...
    this.text.addKeyListener(new KeyAdapter() {
      public void keyPressed(KeyEvent event) {
        for (int i = 0; i < KB_ACTIONS.length; i++) {
          if (event.keyCode == KB_ACTIONS[i].getKeyBinding().getKey()
              && event.stateMask == KB_ACTIONS[i].getKeyBinding().getMask()) {
            TuxGuitar.instance().getAction(KB_ACTIONS[i].getAction()).process(
                event);
            return;
          }
        }
  ...
  private void loadToolBar(Composite parent) {
    ...
    this.fromLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
        true));
  ...
  public void show() {
    ...
    this.track = TuxGuitar.instance().getTablatureEditor().getTablature()
        .getCaret().getTrack();
    this.loadComposites();
    this.loadProperties();
    this.loadIcons();
  ...
  public void updateItems() {
    if (!isDisposed()) {
      ...
      if (isTrackChanged()) {
        this.from.setSelection(this.track.getLyrics().getFrom());
        this.text.setText(this.track.getLyrics().getLyrics());
        this.text.setSelection((this.caretPosition >= 0 ? this.caretPosition
            : this.text.getCharCount()));
      }
