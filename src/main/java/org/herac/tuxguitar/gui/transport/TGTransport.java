...
public class TGTransport implements TGRedrawListener, TGUpdateListener,
  ...
  public void doRedraw(int type) {
    if (type == TGRedrawListener.PLAYING_THREAD
  ...
  public void gotoMeasure(TGMeasureHeader header, boolean moveCaret) {
    if (header != null) {
      ...
      if (playingMeasure == null
          || playingMeasure.getHeader().getNumber() != header.getNumber()) {
        TuxGuitar.instance().getPlayer().setTickPosition(
            MidiTickUtil.getTick(header.getStart()));
        if (moveCaret) {
  ...
  public void gotoNext() {
    ...
    TGMeasureHeader header = getSongManager().getMeasureHeaderAt(
        MidiTickUtil.getStart(player.getTickPosition()));
    if (header != null) {
  ...
  private void initLabel(Composite parent) {
    final Font font = new Font(parent.getDisplay(), "Minisystem", 36,
        SWT.NORMAL);
    this.label = new Label(parent, SWT.RIGHT);
    this.label.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
    this.label.setBackground(parent.getDisplay()
        .getSystemColor(SWT.COLOR_BLACK));
    this.label
        .setForeground(parent.getDisplay().getSystemColor(SWT.COLOR_BLUE));
  ...
  private void initScale(Composite parent) {
    ...
    this.tickProgress = new ProgressBar(parent, SWT.BORDER | SWT.HORIZONTAL
        | SWT.SMOOTH);
    this.tickProgress.setCursor(this.tickProgress.getDisplay().getSystemCursor(
        SWT.CURSOR_HAND));
    ...
    this.tickProgress.addMouseListener(new MouseAdapter() {
      ...
      public void mouseUp(MouseEvent e) {
        gotoMeasure(getSongManager().getMeasureHeaderAt(
            TGTransport.this.tickProgress.getSelection()), true);
  ...
  public void loadPlayText() {
    String property = TuxGuitar
        .getProperty((getStatus() == STATUS_RUNNING ? "transport.pause"
            : "transport.start"));
  ...
  public void loadProperties() {
    if (!isDisposed()) {
      ...
      this.metronome.setToolTipText(TuxGuitar
          .getProperty("transport.metronome"));
          ...
          public void run() {
            if (!isDisposed() && !TuxGuitar.instance().isLocked()) {
              if (isEditingTickScale()) {
                TGTransport.this.label.setText(Long
                    .toString(TGTransport.this.tickProgress.getSelection()));
              } else if (!TuxGuitar.instance().getPlayer().isRunning()) {
                long tickPosition = TuxGuitar.instance().getTablatureEditor()
                    .getTablature().getCaret().getPosition();
  ...
  public void updateItems(boolean force) {
    if (!isDisposed()) {
      ...
      this.tickProgress
          .setMaximum((int) (last.getStart() + last.getLength()) - 1);
  ...
  protected void updateProgressBar(int x) {
    if (isEditingTickScale()) {
      int selection = (this.tickProgress.getMinimum() + ((x * (this.tickProgress
          .getMaximum() - this.tickProgress.getMinimum())) / this.tickProgress
          .getSize().x));
      this.tickProgress.setSelection(Math.max((int) TGDuration.QUARTER_TIME,
          selection));
      this.redraw();
    }
  }
