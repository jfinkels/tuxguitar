...
public class TGDocumentation {
  ...
  private static final String TEMPORAL_PATH = System
      .getProperty("java.io.tmpdir")
      + File.separator + "tuxguitar";

  private void copyTemporalResources(String dstPath, String resourcePath,
      String[] resources) throws Throwable {
    if (resources != null) {
  ...
  public void display() throws Throwable {
    URL url = getIndexUrl();
    if (url != null) {
      Shell dialog = DialogUtils.newDialog(TuxGuitar.instance().getShell(),
          SWT.SHELL_TRIM);
      ...
      DialogUtils.openDialog(dialog, DialogUtils.OPEN_STYLE_CENTER
          | DialogUtils.OPEN_STYLE_MAXIMIZED);
    }
  }
  ...
  private URL getIndexUrl() throws Throwable {
    ...
    if (url != null && !TGFileUtils.isLocalFile(url)) {
      ...
      copyTemporalResources(path, RESOURCE_PATH, TGFileUtils
          .getFileNames(RESOURCE_PATH));
