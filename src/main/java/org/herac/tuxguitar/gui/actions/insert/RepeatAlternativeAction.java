...
public class RepeatAlternativeAction extends Action {
  ...
  public void showCloseRepeatDialog(Shell shell, final TGMeasure measure) {
    if (measure != null) {
      ...
      int selectedEndings = (measure.getHeader().getRepeatAlternative() > 0) ? measure
          .getHeader().getRepeatAlternative()
          : getDefaultEndings(existentEndings);
      ...
      for (int i = 0; i < selections.length; i++) {
        ...
        selections[i].setSelection(enabled
            && ((selectedEndings & (1 << i)) != 0));
      }
  ...
  protected void update(TGMeasure measure, int value) {
    ...
    if (value != measure.getHeader().getRepeatAlternative()) {
      ...
      if (previousRepeatClose) {
        ...
        TGMeasureHeader previous = getSongManager().getMeasureHeader(
            measure.getNumber() - 1);
        if (previous != null && previous.getRepeatClose() == 0) {
          UndoableChangeCloseRepeat u2 = UndoableChangeCloseRepeat.startUndo(
              previous.getStart(), previous.getRepeatClose());
