...
public class ChangeTremoloBarAction extends Action {
  ...
  private void changeTremoloBar(BendingEffect effect) {
    ...
    getSongManager().getMeasureManager().changeTremoloBar(caret.getMeasure(),
        caret.getPosition(), caret.getSelectedString().getNumber(), effect);
  ...
  protected int execute(TypedEvent e) {
    ...
    if (note != null) {
      changeTremoloBar(new TremoloBarEditor().show(getEditor().getTablature()
          .getShell(), note));
    }
    return 0;
  }
