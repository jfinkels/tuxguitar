...
public class TransportModeAction extends Action {
  ...
  private class MHeaderComboController {
    ...
    public MHeaderComboController(MHeaderCombo loopSHeader,
        MHeaderCombo loopEHeader) {
      this.loopSHeader = loopSHeader;
      this.loopEHeader = loopEHeader;
    }

    public void appendListener() {
      this.loopSHeader.getControl().addSelectionListener(
    ...
    public void updateLoopEHeader(int sHeader, int eHeader) {
      ...
      this.loopEHeader.addItem(TuxGuitar
          .getProperty("transport.mode.loop-range.to-default"), -1);
    ...
    public void updateLoopSHeader(int sHeader) {
      ...
      this.loopSHeader.addItem(TuxGuitar
          .getProperty("transport.mode.loop-range.from-default"), -1);
      for (int i = 0; i < song.countMeasureHeaders(); i++) {
        TGMeasureHeader header = song.getMeasureHeader(i);
  ...
  private class SpinnerSelectionAdapter extends SelectionAdapter {
    ...
    public void widgetSelected(SelectionEvent e) {
      ...
      if (this.increment.getSelection() > (this.to.getSelection() - this.from
          .getSelection())) {
        this.increment.setSelection(this.to.getSelection()
            - this.from.getSelection());
      }
    }
  }

  protected static final int[] DEFAULT_PERCENTS = new int[] { 25, 50, 75, 100,
      125, 150, 175, 200 };
  ...
  protected int execute(TypedEvent e) {
    this.showDialog(e.widget.getDisplay().getActiveShell(), TuxGuitar
        .instance().getPlayer().getMode());
    return 0;
  }
  ...
  private Label makeLabel(Composite parent, String text, int aligment,
      int horizontalSpan) {
  ...
  public void showDialog(final Shell parent, final MidiPlayerMode mode) {
    ...
    simpleAdapter.addControl(makeLabel(simpleGroup, TuxGuitar
        .getProperty("transport.mode.simple.tempo-percent"), SWT.LEFT, 1));
    this.simplePercent = new Combo(simpleGroup, SWT.DROP_DOWN | SWT.READ_ONLY);
    this.simplePercent.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    ...
    this.simpleLoop
        .setText(TuxGuitar.getProperty("transport.mode.simple.loop"));
    ...
    customAdapter.addControl(makeLabel(trainerGroup, TuxGuitar
        .getProperty("composition.tempo"), SWT.LEFT, 1));
    ...
    customAdapter.addControl(makeLabel(trainerGroup, TuxGuitar
        .getProperty("edit.to"), SWT.RIGHT, 1));
    ...
    customAdapter.addControl(makeLabel(trainerGroup, TuxGuitar
        .getProperty("transport.mode.trainer.increment-description"), SWT.LEFT,
        4));
    this.customIncrement = new Spinner(trainerGroup, SWT.BORDER);
    this.customIncrement.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
        true));
    ...
    SpinnerSelectionAdapter spinnerAdapter = new SpinnerSelectionAdapter(
        this.customFrom, this.customTo, this.customIncrement);
    ...
    // --- Loop Range ---
    MHeaderRangeStatus mHeaderRangeStatus = new MHeaderRangeStatus(this.simple,
        this.simpleLoop, this.custom);
    ...
    mHeaderRangeStatus.addControl(makeLabel(rangeGroup, TuxGuitar
        .getProperty("transport.mode.loop-range.from"), SWT.LEFT, 1));
    this.loopSHeader = new MHeaderCombo(rangeGroup);
    mHeaderRangeStatus.addControl(this.loopSHeader.getControl());

    mHeaderRangeStatus.addControl(makeLabel(rangeGroup, TuxGuitar
        .getProperty("transport.mode.loop-range.to"), SWT.LEFT, 1));
    this.loopEHeader = new MHeaderCombo(rangeGroup);
    mHeaderRangeStatus.addControl(this.loopEHeader.getControl());

    MHeaderComboController mHeaderController = new MHeaderComboController(
        this.loopSHeader, this.loopEHeader);
    mHeaderController.updateLoopSHeader(mode.getLoopSHeader());
    mHeaderController.updateLoopEHeader(mode.getLoopSHeader(), mode
        .getLoopEHeader());
  ...
  protected void updateMode(MidiPlayerMode mode) {
    int type = (this.custom.getSelection()) ? MidiPlayerMode.TYPE_CUSTOM
        : MidiPlayerMode.TYPE_SIMPLE;
    boolean loop = (type == MidiPlayerMode.TYPE_CUSTOM || (type == MidiPlayerMode.TYPE_SIMPLE && this.simpleLoop
        .getSelection()));
    mode.setType(type);
    mode.setLoop(loop);
    mode
        .setSimplePercent(this.simplePercent.getSelectionIndex() >= 0 ? DEFAULT_PERCENTS[this.simplePercent
            .getSelectionIndex()]
            : MidiPlayerMode.DEFAULT_TEMPO_PERCENT);
    ...
    // mode.setLoopSHeader( ( loop && this.loopSHeader.getSelectionIndex() > 0 ?
    // this.loopSHeader.getSelectionIndex() : -1 ) );
    // mode.setLoopEHeader( ( loop && this.loopEHeader.getSelectionIndex() > 0 ?
    // this.loopEHeader.getSelectionIndex() : -1 ) );
    mode.reset();
  }
}