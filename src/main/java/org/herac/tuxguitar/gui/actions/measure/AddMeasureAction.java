...
public class AddMeasureAction extends Action {
          ...
          new SyncThread(new Runnable() {
            public void run() {
              ...
              for (int i = 0; i < count; i++) {
                ...
                UndoableAddMeasure mUndoable = UndoableAddMeasure
                    .startUndo((number + i));
              ...
              int trackNumber = getEditor().getTablature().getCaret()
                  .getTrack().getNumber();
              int stringNumber = getEditor().getTablature().getCaret()
                  .getStringNumber();
              long start = getSongManager().getMeasureHeader(number).getStart();
              getEditor().getTablature().getCaret().update(trackNumber, start,
                  stringNumber);
  ...
  public void showDialog() {
    if (getEditor().getTablature().getCaret().getMeasure() != null) {
      ...
      beforePosition.setText(TuxGuitar
          .getProperty("measure.add-before-current-position"));

      final Button afterPosition = new Button(options, SWT.RADIO);
      afterPosition.setText(TuxGuitar
          .getProperty("measure.add-after-current-position"));
      ...
      buttonOK.addSelectionListener(new SelectionAdapter() {
        public void widgetSelected(SelectionEvent arg0) {
          ...
          if (beforePosition.getSelection()) {
            number = (getEditor().getTablature().getCaret().getMeasure()
                .getNumber());
