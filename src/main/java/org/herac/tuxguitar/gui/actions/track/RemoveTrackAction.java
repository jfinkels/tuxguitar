...
public class RemoveTrackAction extends Action {
  ...
  protected int execute(TypedEvent e) {
    ...
    TGTrackImpl nextCaretTrack = (TGTrackImpl) getSongManager().getTrack(
        track.getNumber() + 1);
    if (nextCaretTrack == null) {
      nextCaretTrack = (TGTrackImpl) getSongManager().getTrack(
          track.getNumber() - 1);
    }
    ...
    if (nextCaretTrack != null) {
      caret.update(nextCaretTrack.getNumber(), getSongManager()
          .getTrackManager().getMeasureAt(nextCaretTrack,
              caret.getMeasure().getStart()).getStart(), 1);
    }
