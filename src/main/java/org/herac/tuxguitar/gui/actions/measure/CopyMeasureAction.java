...
public class CopyMeasureAction extends Action {
  ...
  protected void copyMeasures(int m1, int m2, boolean allTracks) {
    if (m1 > 0 && m1 <= m2) {
      MeasureTransferable transferable = new MeasureTransferable(getEditor(),
          m1, m2, allTracks);
  ...
  public void showDialog(Shell shell) {
    ...
    if (measure != null) {
      ...
      if (getSongManager().getSong().countTracks() > 1) {
        ...
        checkComposites.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
            true));
      ...
      buttonOK.addSelectionListener(new SelectionAdapter() {
        public void widgetSelected(SelectionEvent arg0) {
          copyMeasures(fromSpinner.getSelection(), toSpinner.getSelection(),
              CopyMeasureAction.this.copyAllTracks);
          dialog.dispose();
        }
      });
