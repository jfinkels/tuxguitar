...
public class ChangeTimeSignatureAction extends Action {
  ...
  protected void setTimeSignature(TGTimeSignature timeSignature, boolean toEnd) {
    ...
    UndoableChangeTimeSignature undoable = UndoableChangeTimeSignature
        .startUndo();

    Caret caret = getEditor().getTablature().getCaret();
    TGMeasureImpl measure = caret.getMeasure();

    getSongManager().changeTimeSignature(measure.getStart(), timeSignature,
        toEnd);
  ...
  public void showDialog(Shell shell) {
    ...
    if (measure != null) {
      ...
      numeratorLabel.setText(TuxGuitar
          .getProperty("composition.timesignature.Numerator"));
      final Combo numerator = new Combo(timeSignature, SWT.DROP_DOWN
          | SWT.READ_ONLY);
      for (int i = 1; i <= 32; i++) {
      ...
      denominatorLabel.setText(TuxGuitar
          .getProperty("composition.timesignature.denominator"));
      final Combo denominator = new Combo(timeSignature, SWT.DROP_DOWN
          | SWT.READ_ONLY);
      for (int i = 1; i <= 32; i = i * 2) {
        denominator.add(Integer.toString(i));
      }
      denominator.setText(Integer.toString(currentTimeSignature
      ...
      toEnd.setText(TuxGuitar
          .getProperty("composition.timesignature.to-the-end"));
