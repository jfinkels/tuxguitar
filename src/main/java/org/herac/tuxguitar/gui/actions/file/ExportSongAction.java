...
public class ExportSongAction extends Action {
  ...
  protected int execute(TypedEvent e) {
    ...
    /*
     * if(!exporter.configure(false)){ return AUTO_UNLOCK; }
     * 
     * final String fileName =
     * FileActionUtils.chooseFileName(exporter.getFileFormat()); if(fileName ==
     * null){ return AUTO_UNLOCK; }
     * 
     * TuxGuitar.instance().loadCursor(SWT.CURSOR_WAIT); new Thread(new
     * Runnable() { public void run() { if(!TuxGuitar.isDisposed()){
     * FileActionUtils.exportSong(exporter, fileName);
     * TuxGuitar.instance().loadCursor(SWT.CURSOR_ARROW); ActionLock.unlock(); }
     * } }).start();
     * 
     * return 0;
     */
  ...
  private int processLocalFileExporter(final TGLocalFileExporter exporter) {
    ...
    final String fileName = FileActionUtils.chooseFileName(exporter
        .getFileFormat());
    if (fileName == null) {
      return AUTO_UNLOCK;
    }
