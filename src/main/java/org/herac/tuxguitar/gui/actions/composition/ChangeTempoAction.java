...
public class ChangeTempoAction extends Action {
  ...
  protected void setTempo(int tempoValue, boolean applyToAllMeasures,
    ...
    if (tempoValue >= MIN_TEMPO && MAX_TEMPO <= 320) {
      ...
      long start = (applyToAllMeasures ? TGDuration.QUARTER_TIME : getEditor()
          .getTablature().getCaret().getMeasure().getStart());
  ...
  public void showDialog(Shell shell) {
    ...
    if (measure != null) {
      ...
      applyToAllMeasures.setText(TuxGuitar
          .getProperty("composition.tempo.start-to-end"));

      final Button applyToEnd = new Button(options, SWT.RADIO);
      applyToEnd.setText(TuxGuitar
          .getProperty("composition.tempo.position-to-end"));

      final Button applyToNext = new Button(options, SWT.RADIO);
      applyToNext.setText(TuxGuitar
          .getProperty("composition.tempo.position-to-next"));
      ...
      buttonOK.addSelectionListener(new SelectionAdapter() {
        ...
        public void widgetSelected(SelectionEvent arg0) {
          ...
          final boolean applyToAllMeasuresValue = applyToAllMeasures
              .getSelection();
