...
public class JackOutputPortRouter {
    ...
    for (int i = 0; i < routing.length; i++) {
      if (routing[i].length == 4) {
        ...
        if (program >= 0 && program < this.programs.length) {
          ...
          this.programs[program][PROGRAM_INDEX] = (routing[i][3] >= 0 ? routing[i][3]
              : program);
        }
        // Percussion Kit
        else if (program == 128) {
  ...
  public void loadSettings(TGConfigManager config) {
    int type = config.getIntConfigValue("jack.midi.ports.type",
        CREATE_UNIQUE_PORT);
    if (type == CREATE_MULTIPLE_PORTS_BY_PROGRAM) {
      ...
      for (int i = 0; i < routing.length; i++) {
        routing[i] = new int[4];
        routing[i][0] = i;
        routing[i][1] = config.getIntConfigValue(
            "jack.midi.port.program-routing.port-" + i, 0);
        routing[i][2] = config.getIntConfigValue(
            "jack.midi.port.program-routing.to-channel-" + i, -1);
        routing[i][3] = config.getIntConfigValue(
  ...
  public void setProgram(int channel, int program) {
    if (channel >= 0 && channel < this.channels.length && program >= 0
