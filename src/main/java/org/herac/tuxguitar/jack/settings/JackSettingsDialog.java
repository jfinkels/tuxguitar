...
public class JackSettingsDialog {
  ...
  protected void addChannelRoutingItem(TableItem item, int[] route) {
    if (route.length == 3) {
      ...
      text[0] = (TuxGuitar
          .getProperty(
              "jack.settings.dialog.options.midi-port.channel-router.src-channel.item",
              new String[] { Integer.toString(route[0]) }));
      if (route[1] >= 0) {
        text[1] = (TuxGuitar
            .getProperty(
                "jack.settings.dialog.options.midi-port.channel-router.dst-channel.item",
                new String[] { Integer.toString(route[1]) }));
      } else {
        text[1] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.dst-channel.default"));
      }
      if (route[2] >= 0) {
        text[2] = (TuxGuitar
            .getProperty(
                "jack.settings.dialog.options.midi-port.channel-router.dst-program.item",
                new String[] { Integer.toString(route[2]) }));
      } else {
        text[2] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.dst-program.default"));
      }
      item.setText(text);
      item.setData(route);
    }
  ...
  protected void addProgramRoutingItem(TableItem item, int[] route) {
    if (route.length == 4) {
      ...
      if (route[0] < 128) {
        text[0] = (TuxGuitar
            .getProperty(
                "jack.settings.dialog.options.midi-port.program-router.src-program.item",
                new String[] { Integer.toString(route[0]) }));
      } else {
        text[0] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.src-program.percussion"));
      }
      if (route[1] > 0) {
        text[1] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-port.dedicated"));
      } else {
        text[1] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-port.default"));
      }
      if (route[2] >= 0) {
        text[2] = (TuxGuitar
            .getProperty(
                "jack.settings.dialog.options.midi-port.program-router.dst-channel.item",
                new String[] { Integer.toString(route[2]) }));
      } else {
        text[2] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-channel.default"));
      }
      if (route[3] >= 0) {
        text[3] = (TuxGuitar
            .getProperty(
                "jack.settings.dialog.options.midi-port.program-router.dst-program.item",
                new String[] { Integer.toString(route[3]) }));
      } else {
        text[3] = (TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-program.default"));
      }
      item.setText(text);
      item.setData(route);
    }
  ...
  public void editChannelRoutingItem(Shell parent, final TableItem item) {
    ...
    dialog
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.options"));
    // ----------------------------------------------------------------------
    ...
    channel
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.src-channel")
            + ":");
    ...
    channelRoute
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.dst-channel")
            + ":");

    final Combo channelRouteValue = new Combo(group, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    channelRouteValue.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
        false));
    channelRouteValue
        .add(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.dst-channel.default"));
    ...
    programRoute
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.dst-program")
            + ":");

    final Combo programRouteValue = new Combo(group, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    programRouteValue.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
        false));
    programRouteValue
        .add(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.channel-router.dst-program.default"));
  ...
  public void editProgramRoutingItem(Shell parent, final TableItem item) {
    ...
    dialog
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.options"));
    // ----------------------------------------------------------------------
    Group group = new Group(dialog, SWT.SHADOW_ETCHED_IN);
    group.setLayout(new GridLayout(2, false));
    group.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
    ...
    program
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.src-program")
            + ":");
    ...
    programPort
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-port")
            + ":");

    final Combo programPortValue = new Combo(group, SWT.DROP_DOWN
        | SWT.READ_ONLY);
    programPortValue.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
        false));
    programPortValue
        .add(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-port.default"));
    ...
    channelRoute
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-channel")
            + ":");
    ...
    programRoute
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.program-router.dst-program")
            + ":");
  ...
  protected int[][] getChannelRoutingSettings() {
    ...
    for (int i = 0; i < channelsRoute.length; i++) {
      ...
      channelsRoute[i][0] = i;
      channelsRoute[i][1] = this.settings.getConfig().getIntConfigValue(
          "jack.midi.port.channel-routing.to-channel-" + i, -1);
      channelsRoute[i][2] = this.settings.getConfig().getIntConfigValue(
          "jack.midi.port.channel-routing.to-program-" + i, -1);
    }
  ...
  protected int[][] getProgramRoutingSettings() {
    ...
    for (int i = 0; i < routing.length; i++) {
      routing[i] = new int[4];
      routing[i][0] = i;
      routing[i][1] = this.settings.getConfig().getIntConfigValue(
          "jack.midi.port.program-routing.port-" + i, 0);
      routing[i][2] = this.settings.getConfig().getIntConfigValue(
          "jack.midi.port.program-routing.to-channel-" + i, -1);
      routing[i][3] = this.settings.getConfig().getIntConfigValue(
          "jack.midi.port.program-routing.to-program-" + i, -1);
    }
    return routing;
  }
  ...
  public void open(Shell parent) {
    ...
    // ----------------------------------------------------------------------
    int synthRouteType = this.settings.getConfig().getIntConfigValue(
        "jack.midi.ports.type", JackOutputPortRouter.CREATE_UNIQUE_PORT);
    ...
    groupSynth.setText(TuxGuitar
        .getProperty("jack.settings.dialog.options.midi-port"));
    ...
    buttonSynthRouteType1.setText(TuxGuitar
        .getProperty("jack.settings.dialog.options.midi-port.type.single"));
    buttonSynthRouteType2
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.type.multiple-by-channel"));
    buttonSynthRouteType3
        .setText(TuxGuitar
            .getProperty("jack.settings.dialog.options.midi-port.type.multiple-by-program"));
    ...
    final SelectionListener routeTypeListener = new SelectionAdapter() {
      public void widgetSelected(SelectionEvent arg0) {
        boolean channelRoutingTabsEnabled = buttonSynthRouteType2
            .getSelection();
        boolean programRoutingTabsEnabled = buttonSynthRouteType3
            .getSelection();
  ...
  protected void saveChanges(int type, int[][] channelRouting,
      int[][] programRouting) {
  ...
  protected void setChannelRoutingSettings(int[][] channelsRoute) {
    for (int i = 0; i < channelsRoute.length; i++) {
      if (channelsRoute[i].length == 3) {
        this.settings.getConfig().setProperty(
            "jack.midi.port.channel-routing.to-channel-" + channelsRoute[i][0],
            channelsRoute[i][1]);
        this.settings.getConfig().setProperty(
            "jack.midi.port.channel-routing.to-program-" + channelsRoute[i][0],
            channelsRoute[i][2]);
      }
    }
  }
  ...
  protected void setProgramRoutingSettings(int[][] routing) {
    for (int i = 0; i < routing.length; i++) {
      if (routing[i].length == 4) {
        this.settings.getConfig().setProperty(
            "jack.midi.port.program-routing.port-" + routing[i][0],
            routing[i][1]);
        this.settings.getConfig().setProperty(
            "jack.midi.port.program-routing.to-channel-" + routing[i][0],
            routing[i][2]);
        this.settings.getConfig().setProperty(
            "jack.midi.port.program-routing.to-program-" + routing[i][0],
            routing[i][3]);
      }
    }
  }
