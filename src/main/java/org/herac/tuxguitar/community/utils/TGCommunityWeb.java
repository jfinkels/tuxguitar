...
public class TGCommunityWeb {
  ...
  public static String HOME_URL = TGCommunitySingleton.getInstance()
      .getConfig().getStringConfigValue("community.url");
        ...
        if (browserCmd != null && browserCmd.length() > 0) {
          ...
          if (indexOfPattern >= 0) {
            ...
            if (browserCmd.length() > (indexOfPattern + pattern.length())) {
              commandLine += (browserCmd.substring((indexOfPattern + pattern
                  .length()), browserCmd.length()));
            }
  ...
  private static boolean openDesktopBrowser(URL url) {
    try {
      ...
      if (desktopClass != null) {
        Method desktop_getDesktop = desktopClass.getDeclaredMethod(
            "getDesktop", new Class[] {});
        Method desktop_browse = desktopClass.getDeclaredMethod("browse",
            new Class[] { java.net.URI.class });
        if (desktop_getDesktop != null && desktop_browse != null) {
          Object desktopObject = desktop_getDesktop.invoke(null,
              new Object[] {});
          if (desktopObject != null) {
            desktop_browse.invoke(desktopObject,
                new Object[] { new java.net.URI(url.toExternalForm()) });
          }
        }
      }
      return true;
