            ...
            if (name != null && name.trim().length() > 0) {
              ...
              for (int c = 0; c < nodeChildren.getLength(); c++) {
                ...
                if (child.getNodeName().equals(TAG_SONG)) {
                  element.setUrl(getAttributeValue(child.getAttributes(),
                      ATTRIBUTE_URL));
                  ...
                  for (int p = 0; p < parameters.getLength(); p++) {
                    ...
                    if (parameter.getNodeName().equals(TAG_PARAMETER)) {
                      NamedNodeMap parameterAttributes = parameter
                          .getAttributes();

                      String key = getAttributeValue(parameterAttributes,
                          ATTRIBUTE_KEY);
                      String value = getAttributeValue(parameterAttributes,
                          ATTRIBUTE_VALUE);
