...
public class TGCommunityAuthDialog {
  ...
  protected void doOpen(Shell shell) {
    ...
    passwordLabel.setText(TuxGuitar
        .getProperty("tuxguitar-community.auth-dialog.signin.password")
        + ":");
    ...
    join.setLayoutData(new GridData(
        group.computeSize(SWT.DEFAULT, SWT.DEFAULT).x, SWT.DEFAULT));
    join.setText(TuxGuitar
        .getProperty("tuxguitar-community.auth-dialog.signup"));

    final Link joinLink = new Link(join, SWT.LEFT);
    joinLink.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, true));
    joinLink.setText(TuxGuitar
        .getProperty("tuxguitar-community.auth-dialog.signup.tip"));
